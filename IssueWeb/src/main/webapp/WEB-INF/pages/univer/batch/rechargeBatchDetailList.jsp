...
if(${flag}==1){
		TimeOperation(${order_id},
				...
				"${ctx}/batchfile/getRechargeBatchDetail.action?order_id="+${order_id}+"&batch_id="+${batch_id},
				'充值中,请耐心等候...','5s');
	} else if (${flag}==2){
		
		  location.href="${ctx}/batchfile/getRechargeBatchDetail.action?order_id="+${order_id}+"&batch_id="+${batch_id};  
	}
}
function changeCard(){
	cardNo = $("#cardNo").val()
	...
	$("#batchForm").attr("action","${ctx}/batchfile/changeCardNo.action")
	$("#batchForm").submit()
	//window.close()
//	alert(${"#order_id"}.val())
}
function toBack(){
	 location.href="${ctx}/orderImmediatelyCreditAction!list.action";  
}
</script>
	</head>
	<body onload="onload();">
		...
		<div id="list"
			...
			<div id="listTable"
				...
				<s:form id="batchForm" name="batchForm"
					...
					<s:hidden id="order_id" name="order_id"></s:hidden>
								...
								<div id="QueryBody">
									...
									<table width="100%" height="30" border="0" cellpadding="0"
										...
										<tr height="35">
											...
											<td width="180">
												<s:textfield name="sellOrderQueryDTO.cardNo" id="cardNo"
													></s:textfield>
												&nbsp;
											</td>
