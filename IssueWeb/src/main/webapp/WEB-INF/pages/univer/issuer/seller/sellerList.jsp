...
function queryData() {
	var sellerName = document.getElementById("queryForm_sellerQueryDTO_sellerName").value;
	//document.getElementById("queryForm_sellerQueryDTO_sellerName").value = encodeURIComponent(sellerName); 
	queryForm.submit();
}
</script>
<body>
...
<%--                        </td>--%>
                    ...
                    <tr>
                        ...
                        <td align="right" colspan="2">
                            <table border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                    <td>
                                        <input type="button" class="bt" style="margin: 5px" onclick="queryData()" value="查 询"/>
                                    </td>
                                </tr>
                            </table>
                        </td>
    ...
    <div id="list" style="border: 1px solid #B9B9B9; margin: 20px 8px 0px;">
        ...
        <div id="listTable" style="background-color: #FBFEFF; border-style: solid none none; border-width: 1px; border-color: #B9B9B9;">
            <s:form id="customerForm" name="customerForm" action="issuerSeller/inquery.action" method="post">
                ...
                <ec:table items="pageDataDTO.data" var="map" width="100%"
                    form="customerForm" 
                    action="${ctx}/issuerSeller/inquery.action"
                    imagePath="${ctx}/images/extremecomponents/*.gif"
                    view="html"
                    retrieveRowsCallback="limit"
                    autoIncludeParameters="false">
                    <ec:row>
                        <ec:column property="null" alias="sellerIds" title="选择" width="5%"  headerCell="selectAll">
                        	<s:if test="#attr['map'].dataState == 1">
                            	<input type="checkbox" name="sellerIds" value="${map.entityId},${map.fatherEntityId},${map.userId}" />
                            </s:if>
                        </ec:column>
                        <ec:column property="entityId" title="营销机构号" width="10%" >
                            <a href="${ctx}/issuerSeller/view.action?sellerDTO.entityId=${map.entityId}&sellerDTO.fatherEntityId=${map.fatherEntityId}">
                            	${map.entityId}
                            </a>
                        </ec:column>
                    ...
                    <tr>
                        <td align="right">
                            <table border="0" cellpadding="0" cellspacing="0" >
                                <tr>
                                 <td>
											<display:security urlId="30704">
												<input type="button" class="btn"
													style="width: 100px; height: 20px; margin: 2px 5px; background: url(${ctx}/images/icon/modify.gif) no-repeat; text-align: right"
													onclick="submitForm('customerForm', '${ctx}/issuerSeller/sellerModifyUserState.action', 'edit', 'sellerIds')"
													value="编辑机构用户" />
											</display:security>
										</td>
										<td>
											<display:security urlId="30705">
												<input type="button" class="btn"
													style="width: 100px; height: 20px; margin: 2px 5px; background: url(${ctx}/images/icon/add.gif) no-repeat; text-align: right"
													onclick="submitForm('customerForm', '${ctx}/issuerSeller/modifyPassword.action', 'edit','sellerIds')"
													value="修改用户密码" />
											</display:security>
										</td>
										<td>
											<display:security urlId="30702">
												<input type="button" class="btn"
													style="width: 50px; height: 20px; margin: 2px 5px; background: url(${ctx}/images/icon/add.gif) no-repeat; text-align: right"
													onclick="submitForm('customerForm', '${ctx}/issuerSeller/add.action', 'add')"
													value="添加" />
											</display:security>
										</td>
										<td>
											<display:security urlId="30703">
												<input type="button" class="btn"
													style="width: 50px; height: 20px; margin: 2px 5px; background: url(${ctx}/images/icon/modify.gif) no-repeat; text-align: right"
													onclick="submitForm('customerForm', '${ctx}/issuerSeller/edit.action', 'edit', 'sellerIds')"
													value="编辑" />
											</display:security>
										</td>
										<td>
											<display:security urlId="30706">
												<input type="button" class="btn"
													style="width: 50px; height: 20px; margin: 2px 5px; background: url(${ctx}/images/icon/delete.gif) no-repeat; text-align: right"
													onclick="submitForm('customerForm', '${ctx}/issuerSeller/delete.action', 'delete', 'sellerIds')"
													value="注销" />
											</display:security>
										</td>
										<td>
											<display:security urlId="30707">
												<input type="button" class="btn"
													style="width: 70px; height: 20px; margin: 2px 5px; background: url(${ctx}/images/icon/modify.gif) no-repeat; text-align: right"
													onclick="submitForm('customerForm', '${ctx}/issuerSeller/modifyState.action', 'edit', 'sellerIds')"
													value="修改状态" />
											</display:security>
										</td> 
                                </tr>
