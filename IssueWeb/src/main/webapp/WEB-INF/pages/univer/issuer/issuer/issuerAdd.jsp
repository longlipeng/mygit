	   ...
	   // var y=${isHuaShu};
	     ...
	     if(y=="1"){
	        $(function(){
	            ...
	            $("#entityId").val("请输入四位数字前缀").css({"color":"#888"});
	            $("#entityId").focus(function(){
	                 var oldVal=$(this).val(); 
	                 if(oldVal=="请输入四位数字前缀"){
	           ...
	           $("#entityId").blur(function(){
	                 var oldVal=$(this).val(); 
	                 if(oldVal=="" || oldVal.length<4 ||(/(^0+)/.test(oldVal))){
	                   $(this).val("请输入四位数字前缀").css({"color":"#888"});
	                   return;
	                 }else{
	                     ajaxRemote('${ctx}/checkIssuerIdPin.action',
      	                   {'issuerDTO.issuerIdPin':oldVal},
      	                     succFn,
      	                     'json'
      	                 )
	                 }  
	    ...
	    function succFn(issuerDTO){
      		if(issuerDTO!=null){
      			$("#errorMsg").html("发卡机构前缀"+issuerDTO['issuerIdPin']+"已存在！");
      			$("#entityId").val("请输入四位数字前缀").css({"color":"#888"});
      			return;
      		}else{
      		   $("#errorMsg").html("此前缀可用").css({"color":"#0f0"});
      		}
		...
		<script type="text/javascript"  >
          ...
          function sub(){
            ...
            if(checkid!=null){
        	  ...
        	  maskDocAll();
        	  newForm.action="issuersInsert.action";
        	  newForm.submit(); 
          }
          ...
          function record(){
             var id=window.showModalDialog('${ctx}/record', '_blank', 'center:yes;dialogHeight:400px;dialogWidth:600px;resizable:no');
              newForm.action='configEntiyId.action?issuerQueryDTO.entityId='+id;      
              newForm.submit();
          }
          </script>
  				...
  				if(parseInt(menuList[i]['fatherResourceId'])==0){
  					...
  					switch (parseInt(menuList[i]['resourceType'])) {
  						...
  						case 2:
							...
							/* if(nodeObject[menuList[i]['fatherResourceId']] == undefined){
 								alert(menuList[i]['fatherResourceId']);
							} */
											...
											<table style="text-align: left; width: 100%">
												<tr>
													<td style="width: 120px; text-align: right;">
														<span class="no-empty">*</span>发行机构ID号：
													</td>
													<td>
														<s:textfield name="issuerDTO.entityId"   id="entityId" maxlength="8" ></s:textfield>
														<!--  <button class='bt' type="button" style="margin: 5px"
				                                              onclick="record();">
				                                                                                                                       录入
			                                            </button> -->
														<span class="errorMessage" id="errorMsg" />
										...
										<td>
											<table style="text-align: left; width: 100%">
												<tr>
													...
													<td>
													   <s:textfield name="issuerDTO.issuerName"></s:textfield>
												       <br/><s:fielderror>
															<s:param> 
...
<%--													</td>--%>
<%--													<td>--%>
<%--														<s:textfield name="issuerDTO.issuerCode" />--%>
<%--														<s:fielderror>--%>
<%--															<s:param>--%>
...
<%--										</td>--%>
										...
										<td>
											...
											<table style="text-align: left; width: 100%">
												<tr>
													...
													<td>
														<s:select list="#{1:'有效',0:'无效'}" name="issuerDTO.dataState"/>
													</td>
												</tr>
											</table>
										</td>				
				                        ...
				                        <td>
				                            <table style="text-align: left; width: 100%">
				                                   <tr>
				                                    ...
				                                    <td>
				                                    		<s:textfield id="userName" name="issuerDTO.userName" maxlength="128" onblur="checkUserName(this.value)"></s:textfield>
				                                       	 	<span class="errorMessage" id="message" />
				                                    </td>
				                                 	<td>
				                                    	 <s:fielderror>
				                                            <s:param>issuerDTO.userName</s:param>
				                                        </s:fielderror>
				                              		</td>
				                                </tr>
				                            </table>
										...
										<td colspan="2">
											<table style="text-align: left; width: 100%">
												<tr>
													...
													<td>
														<s:textarea name="issuerDTO.issuerComment" cols="70" rows="5"/>
														<s:fielderror>
															<s:param>
		...
		<div id="btnDiv" style="text-align: right; width: 100%">
			<input type="button" class='bt' style="float: right; margin: 5px 10px"
			value = "返 回" onclick="back();" />
				 
			
			<input type="button" class='bt' style="float: right; margin: 5px"
				value = "提 交" onclick="maskDocAll();sub();" />
	...
	<div id="bankTitle"
		...
		<div id="invoiceCompanyTable"
			...
			<s:if test="issuerDTO.entitySystemParameterDTOs!=null">
		<div id="invoiceCompanyTitle"
			style="font-weight: bold; height: 19px; background-color: #DFE8F6;">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td class="TableTitleFront" style="cursor: pointer">
						<span class="TableTop">密钥信息</span>
					</td>
					<td class="TableTitleEnd">
						&nbsp;
					</td>
	...
	<s:form id="entitySystemParameterForm" name="entitySystemParameterForm" action="" method="post">
		<s:hidden name="issuerDTO.entityId"/>
		<div id="invoiceCompanyTable"
			style="background-color: #FBFEFF; padding: 5px; border-style: solid none none; border-width: 1px; border-color: #B9B9B9;">
             <ec:table tableId="cardSerialtable" items="issuerDTO.entitySystemParameterDTOs"
				var="map" width="100%" form="EditForm"
				imagePath="${ctx}/images/extremecomponents/*.gif" view="html"
				autoIncludeParameters="false" showPagination="false"
				sortable="false">
				<ec:row>
					<ec:column property="parameterName" title="密钥名称" width="50%"></ec:column>
					<ec:column property="parameterValue" title="密钥值" width="40%"></ec:column>
					<ec:column property="null" title="编辑" width="10%" >
                   <a href="javascript:updateParameter('${map.parameterCode}');">编辑</a>
               </ec:column>
				</ec:row>
			</ec:table>
		</div>
