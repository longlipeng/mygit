...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body onload="load();">
		...
		<s:form id="validRuleForm" name="validRuleForm"
			action="/validPeriodRule/validPeriodRuleAction!insert.action" method="post">
			<div id="validRule"
								...
								<table style="text-align: left; width: 100%">
									<tr>
										...
										<td>
											...
											<s:fielderror>
												<s:param>validPeriodRuleDspDTO.ruleDspId</s:param>
											</s:fielderror>
										</td>
									</tr>
								</table>
							...
							<td>
								<table style="text-align: left; width: 100%">
									<tr>
										...
										<td>
											<s:textfield name="validPeriodRuleDspDTO.ruleName" id="ruleName"/>
										</td>
										<td>
											
											<s:fielderror>
												<s:param>validPeriodRuleDspDTO.ruleName</s:param>
											</s:fielderror>
										</td>
									</tr>
								</table>
							...
							<td>
								<table style="text-align: left; width: 100%">
									<tr>
										...
										<td>
											<s:textarea name="validPeriodRuleDspDTO.ruleDsp" cols="20" rows="5" />
											 <s:fielderror>
				...
				<div
					...
					<s:if test="validPeriodRuleQueryDTO.list==null">	
					<div id="validRuleAmountTable" style="width: 100%;">								
						<s:hidden name="ruleStep"/>
						<s:hidden name="ruleInfId" value=""></s:hidden>
						<s:hidden name="dataState" value=""></s:hidden>
						<table id="amountTable" width="800px;">
							<tr>
								<td>
									<s:textfield name="amountMin" value="0" readonly="true" cssClass="readonly" maxlength="10"/>&nbsp;元
								</td>
								<td>
									<s:textfield name="amountMax" onchange="changeAmountMin(this);" maxlength="10"/>&nbsp;元
								</td>
								<td>
									<s:textfield name="delayMonth" />&nbsp;月
								</td>
							</tr>
						</table>
						</div>
					...
					<div id="validRuleAmountTable${status.index}" style="width: 100%;">
						...
						<table id="amountTable" width="800px;">
							<tr>
								<td>
									<input type="text" name="amountMin" readonly="readonly" class="readonly" maxlength="10" value="${minAmount }"/>&nbsp;元
								</td>
								<td>
			...
			<div id="buttonDiv" style="margin: 5px 8px 0px;">
				<script type="text/javascript">
					...
					if(size!=null&&size!=""){
						a=parseInt(size);
						 feeRuleDiv =document.getElementById("validRuleAmountTable"+(a-1));
						 document.getElementById("listSize").value=a+1;
					}else{
					 feeRuleDiv =document.getElementById("validRuleAmountTable");
					}
					 var div = document.createElement("DIV"); 
						div.innerHTML=feeRuleDiv.innerHTML;		
						a+=1;		
					 div.id="validRuleAmountTable"+(a-1);						
	...
	function commit() {
		...
		if(transType==1){
			...
			for ( var i = 0; i < amountMins.length; i++) {
				...
				if ((!isNullOrEmpty(amountMaxs[i].value))
						&& !amountPatrn.exec(amountMaxs[i].value)) {
					errorDisplay("计算截至金额整数部分不超过7位,保留两位小数");
					amountMaxs[i].value = "";
					amountMaxs[i].focus();
					return;
				}
				if ((!isNullOrEmpty(amountMins[i].value))
						&& (!isNullOrEmpty(amountMaxs[i].value))) {
				...
				if ((!isNullOrEmpty(delayMonth[i].value))
						&& !monthPatrn.exec(delayMonth[i].value)) {
