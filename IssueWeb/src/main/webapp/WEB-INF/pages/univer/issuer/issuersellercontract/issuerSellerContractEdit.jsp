...
<title>编辑营销机构合同</title>
<script type="text/javascript">
    function choiceSeller() {
        var sellerDTO = window.showModalDialog('${ctx}/seller/choiceForSelf.action', '_blank', 'center:yes;dialogHeight:600px;dialogWidth:800px;resizable:no');
        if (sellerDTO != null) {
	...
	function addProduct(){
		...
		var contractBuyer = document.getElementById("contractBuyer").value;
		var returnValue = window.showModalDialog('${ctx}/issuerSellerConstract/addProductService.action?sellerContractDTO.sellContractId='+sellerContractId+'&sellerContractDTO.contractBuyer='+contractBuyer+'&sellerContractDTO.contractSeller=${sellerContractDTO.contractSeller}', '_blank', 'center:yes;dialogHeight:600px;dialogWidth:700px;resizable:no');
		if(returnValue == 'success'){
			document.productContractFrom.action="${ctx}/issuerSellerConstract/edit.action";
	...
	function sub(){
    	...
    	document.contractForm.action = '${ctx}/issuerSellerConstract/update.action';
...
<body>
    ...
    <div class="TitleHref">
        <span>编辑营销机构合同信息</span>
    </div>
    <s:form id="contractForm" name="contractForm" action="issuerSellerConstract/edit.action" method="post">
    	<s:token></s:token>
    	<s:hidden id="sellerContractDTO.sellContractId" name="sellerContractDTO.sellContractId" />
    	<!--<s:hidden id="sellerContractDTO.contractBuyer" name="sellerContractDTO.contractBuyer" />-->
	    ...
	    <s:if test="productContractDTOs != null">
	    	<s:form id="productContractFrom" name="productContractFrom" action="" method="post">
		    	...
		    	<s:iterator value="productContractDTOs" id="map" var="map">
		    		...
		    		--%><!-- 服务明细 -->
			        <s:include value="issuerSellerProductView.jsp" />
			                ...
			                <tr>
			                    <td align="right">
			                        <table border="0" cellpadding="0" cellspacing="0">
			                            <tr>
			                                <td>
			                                    <input type="button" class="bt" style="margin: 5px" onclick="submitForm('productContractFrom', '${ctx}/sellerConstract/editProduct.action?sellerProductContractDTO.id='+${map.id})" value="编辑"/>
			                                </td>
			                            </tr>
			                        </table>
			                    </td>
