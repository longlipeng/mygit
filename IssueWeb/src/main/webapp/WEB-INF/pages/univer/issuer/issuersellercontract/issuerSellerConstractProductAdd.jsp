...
<script type="text/javascript">
    ...
    function choiceProduct() {
        var productId = window.showModalDialog(
                '${ctx}/issuerSellerConstract/productChoice.action?issuerFlag=1&sellerContractQueryDTO.sellContractId=${sellerContractDTO.sellContractId}&sellerContractQueryDTO.contractBuyer=${sellerContractDTO.contractSeller}', 
                '_blank', 'center:yes;dialogHeight:600px;dialogWidth:700px;resizable:yes');
        if (productId != null) {
        	document.productForm.action = '${ctx}/issuerSellerConstract/productView.action?productDTO.productId='+productId;
    ...
    function choiceRule(selectAcctypeNo,serviceId) {
    	...
    	var ruleNo = window.showModalDialog('${ctx}/issuerSellerConstract/serviceRuleChoice.action','_blank', 'center:yes;dialogHeight:400px;dialogWidth:600px;resizable:no');
		if(ruleNo != null){
    ...
    function addIssuerProductService(){
		...
		document.productForm.action = '${ctx}/issuerSellerConstract/insertProductService.action';
    	document.productForm.submit();
		
    }
    function initFee() {
    	if(document.getElementById('cardFee').value==""){
       	...
       	<ec:table items="productServices" var="accTypeId" width="100%"
            ...
            <ec:row>
               ...
               <ec:column property="null" title="账户计算规则" width="15%" >
               		<input type="hidden" name="ruleNoList" value="${defaultRuleNo}"></input>
               		<input type="text" value="${defaultRuleNo}" id="ruleNo_${ROWCOUNT-1}" name="ruleNo_${ROWCOUNT-1}" class="watch" onclick="choiceRule(${ROWCOUNT-1},${productServices[ROWCOUNT-1].serviceId})" />
                </ec:column>
            </ec:row>
        </ec:table>
