...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	function updateOrigCardState(state){
		var flag = true;
		var inputStr;
		var inputArr;
		var cardState;
		for(i = 0; i < document.getElementsByName("origCardListStr").length; i++) {
			if (document.getElementsByName("origCardListStr").item(i).checked) {
				flag = false;
				inputStr = document.getElementsByName("origCardListStr").item(i).value;
				inputArr = inputStr.split(",");
				cardState=inputArr[1];
			}
		}
		if(flag){
		...
		if(state=='1' && cardState=='3'){
			alert('无效操作');
			return;
		}
		if(state=='3' && cardState=='0'){
			alert('请先验收卡片');
			return;
		}
		document.origCardviewForm.action="${ctx}/orderReadyAction!updateOrigCardState?sellOrderOrigCardListDTO.cardSate="+state;
	...
	function orderListAllReady(){
		document.newForm.action='${ctx}/orderReadyAction!allReadyForChangeCard?orderReadyDTO.orderId=${sellOrderDTO.orderId}&orderReadyDTO.productId=${sellOrderDTO.productId}&orderReadyDTO.processEntityId=${sellOrderDTO.processEntityId}', '_blank', 'center:yes;dialogHeight:600px;dialogWidth:1200px;resizable:no';
		document.newForm.submit();
	}
	function deleteRecord(orderCardId){
				newForm.action="${ctx}/orderReadyAction!deleteRecordForChangeCard?orderReadyDTO.orderId=${sellOrderDTO.orderId} & orderReadyDTO.processEntityId=${sellOrderDTO.processEntityId}";
	...
	function cardReady(){
				...
				var returnValue =window.showModalDialog('${ctx}/orderReadyAction!cardReady?orderReadyDTO.orderId=${sellOrderDTO.orderId}&orderReadyDTO.orderType=${sellOrderDTO.orderType}&orderReadyDTO.productId=${sellOrderDTO.productId}&orderReadyDTO.processEntityId=${sellOrderDTO.processEntityId}&orderReadyDTO.orderListId='+orderListId, '_blank', 'center:yes;dialogHeight:600px;dialogWidth:1200px;resizable:no');
				if(returnValue==1){			 
				  	document.forms[0].action = "orderReadyAction!readyChangeCard";
  ...
  <body onload="onOperChange();onBankChange();onCusBankChange()">
	...
	<s:form id="newForm" name="newForm"
		...
		<div style="width: 100%" align=center>
								...
								<ec:table items="origCardList" var="map" width="100%" form="origCardviewForm"
									...
									<ec:column property="null" alias="origCardListStr" title="选择"
										...
										<input type="checkbox" name="origCardListStr"
											value="${map.origCardListId},${map.cardState }" />
									</ec:column>
			...
			<table width="98%" height="20" border="0" cellpadding="0"
				cellspacing="0">
				<tr  bgcolor="#FFFFFF">
						<td colspan="4" style="padding: 5px;" align="right">
							<button class='btn'
								onclick="updateOrigCardState(1);">
								验收
							</button>&nbsp;&nbsp;&nbsp;&nbsp;
<!--							<button class='btn'-->
<!--								onclick="updateOrigCardState(2);">-->
<!--								入库-->
<!--							</button>&nbsp;&nbsp;&nbsp;&nbsp;-->
							<button class='btn'
								onclick="updateOrigCardState(3);">
								销户
							</button>
					</td>
				</tr>
					...
					<tr>
						<td width="98%" height="10" align="center" valign="middle"
							...
							<div id="TableBody">
								...
								<ec:column property="serviceName" title="充值账户" width="10%" sortable="false"/>
								...
								<ec:column property="validityPeriod" title="有效期" width="11%" sortable="false"/>
								<ec:column property="cardAmount" title="张数" width="7%" sortable="false"/>
								<ec:column property="realAmount" title="张数" width="7%" sortable="false"/>
								</ec:row>
								</ec:table>
								</s:form>
							</div>
							...
							<div id="orderCardListBody">
								...
								<ec:table items="orderCardList" var="map" width="100%" form="orderCardListForm"
									...
									<ec:row ondblclick="">
										...
										<ec:column property="null"  title="操作" width="15%" sortable="false" >
