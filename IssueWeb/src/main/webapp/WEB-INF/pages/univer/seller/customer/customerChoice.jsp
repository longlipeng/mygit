...
<body>
    ...
    <div id="query" style="border: 1px solid #B9B9B9; margin: 5px 8px 0px;">
        ...
        <div id="queryTable" style="background-color: #FBFEFF; padding: 5px; border-style: solid none none; border-width: 1px; border-color: #B9B9B9;">
            <s:form id="queryForm" name="queryForm" action="customer/choicePer.action" method="post">
                ...
                <table width="100%" style="table-layout: fixed;">
                    <tr>
                     ...
                     <table style="text-align: left; width: 100%">
                          <tr>
                              <td width="90" align=right>证件号码：</td>
    ...
    <div id="list" style="border: 1px solid #B9B9B9; margin: 20px 8px 0px;">
        ...
        <div id="listTable" style="background-color: #FBFEFF; padding: 5px; border-style: solid none none; border-width: 1px; border-color: #B9B9B9;">
            <s:form id="customerForm" name="customerForm" action="customer/choicePer.action" method="post">
                ...
                <s:hidden name="customerQueryDTO.cusState"></s:hidden>
                ...
                <ec:table items="pageDataDTO.data" var="map" width="100%"
                    form="customerForm"
                    action="${ctx}/customer/choicePer.action"
                    ...
                    <ec:row>
                        <ec:column property="null" alias="customerIdList" title="选择" width="5%" sortable="false" headerCell="">
                        	<s:if test="#attr['map'].dataState == 1">
                            	<input type="radio" name="customerIdList" value="${map.entityId},${map.fatherEntityId},${map.customerType}" />
                            </s:if>
                        </ec:column>
                        <ec:column property="entityId" title="客户号" width="10%">
                            <a href="${ctx}/customer/view.action?customerDTO.entityId=${map.entityId}&customerDTO.fatherEntityId=${map.fatherEntityId}&verifierViewFlag=2">
                            	${map.entityId}
                            </a>
                        </ec:column>
                        ...
                        <ec:column property="cusState" title="有效状态" width="10%">
                            <s:if test="#attr['map'].cusState == 1">已审核</s:if>
                            <s:if test="#attr['map'].cusState == 4">审核中</s:if>
                            <s:if test="#attr['map'].cusState == 3">审核未通过</s:if>
                            <s:if test="#attr['map'].cusState == 0">无效</s:if>
                            <s:if test="#attr['map'].cusState == 2">未审核</s:if>
                        </ec:column>
