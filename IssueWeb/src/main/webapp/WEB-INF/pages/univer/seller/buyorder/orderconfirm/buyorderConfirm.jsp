...
<html>
	...
	<body>
					...
					<td rowspan="2">
						<div id="btnDiv" style="text-align: right; width: 300px;">
							<button class='btn' style="float: right; margin: 40px 7px 2px"
								onclick="window.open('${ctx}/${actionName}!listBuyOrder', '_self')">
									返回
							</button>
							<button class='btn' style="float: right; margin: 40px 7px 2px"
								onclick="submitForm();">
		...
		<s:if test="getSellOrderDTO().getOrderType()==20000001">
			<%@ include file="../../order/orderview/buyOrderSignView.jsp"%>
		</s:if>
		
		<s:if test="getSellOrderDTO().getOrderType()==20000002">
			<%@ include file="../../order/orderview/buyOrderUnsignView.jsp"%>
		</s:if>
