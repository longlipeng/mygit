...
<script type="text/javascript" >
	function submitForm(method) {
		if("add" == method){
			document.queryForm.action="${ctx}/sellerConstract/add.action";
		...
		if (method=="edit" && !flag) {
			document.queryForm.action="${ctx}/sellerConstract/edit.action";
			document.queryForm.submit();
			return;
		}
...
<title>营销机构合同信息管理</title>
</head>
<body>
    <%@ include file="/commons/messages.jsp"%>
    <div class="TitleHref">
        <span>营销机构合同信息管理</span>
    </div>
    <div id="query" style="border: 1px solid #B9B9B9; margin: 5px 8px 0px">
        ...
        <div id="queryTable" style="background-color: #FBFEFF; padding: 5px; border-style: solid none none; border-width: 1px; border-color: #B9B9B9;">
            <s:form id="queryForm" name="queryForm" action="sellerConstract/inquery.action" method="post">
                <table width="100%" style="table-layout: fixed;">
                    <tr>
                        <td>
                            <table style="text-align: left; width: 100%">
                                <tr>
                                    <td style="width: 110px; text-align: right;">营销机构号：</td>
                        ...
                        <td>
                            <table style="text-align: left; width: 100%">
                                <tr>
                                    <td style="width: 110px; text-align: right;">营销机构合同号：</td>
...
<!--                                    	<s:textfield name="sellerContractQueryDTO.ruleType"/>-->
										 <s:select  list="#{'':'--请选择--','1':'汇总结算'}"
												...
												id="sellerContractQueryDTO.ruleType" 
												label="结算类型：" />
                                        <s:fielderror>
    ...
    <div id="list" style="border: 1px solid #B9B9B9; margin: 20px 8px 0px">
        ...
        <div id="listTable" style="background-color: #FBFEFF; padding: 5px; border-style: solid none none; border-width: 1px; border-color: #B9B9B9;">
                <ec:table items="pageDataDTO.data" var="map" width="100%"
                    form="queryForm"
                    action="${ctx}/sellerConstract/inquery.action"
                    imagePath="${ctx}/images/extremecomponents/*.gif"
                    view="html"
                    retrieveRowsCallback="limit"
                    autoIncludeParameters="false">
                    <ec:row>
                        <ec:column property="null" alias="sellerContractIds" title="选择" width="10%" sortable="false" headerCell="selectAll">
                            	<input type="checkbox" name="sellerContractIds" value="${map.sellContractId}" />
                        </ec:column>
                        <ec:column property="sellContractId" title="合同号" width="10%" >
                            <a href="sellerConstract/view.action?sellerContractDTO.sellContractId=${map.sellContractId}">
                                <s:property value="#attr['map'].sellContractId" />
                            </a>
                        </ec:column>
                        <ec:column property="contractBuyer" title="营销机构" width="10%" />
