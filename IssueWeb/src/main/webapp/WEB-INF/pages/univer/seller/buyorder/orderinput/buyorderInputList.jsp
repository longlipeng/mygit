...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script type="text/javascript">
			...
			function submitForm(method) {
				...
				if(method=="add") {
					var contractState = ${sellOrderQueryDTO.contractState};
					 if(contractState=="99999"){
						errorDisplay("该营销机构的无合同，不允许添加新订单！");
						return;
					} 
					...
					EditForm.action = "buyOrderUnSignAction!addBuyOrder";
					EditForm.submit();
					return;
				}
				var flag = true;
				...
				for(i = 0; i < document.getElementsByName("ec_choose").length; i++) {
					if (document.getElementsByName("ec_choose").item(i).checked) {
						if(method=="submit") {
							document.queryForm.action = "orderInputAction!"+method+"?buyOrderFlag=1";
			...
			function operation(){
				document.queryForm.action = "orderInputAction!cancel?buyOrderFlag=1";
				document.queryForm.submit();
			}
		</script>
	</head>
	<body>
		...
		<div class="TitleHref">
			<span>订单管理-->采购订单录入</span>
		</div>
		
		<div id="ContainBox">
		<s:form id="queryForm" name="queryForm"
								action="orderInputAction!listBuyOrder">
				...
				<tr>
					<td width="98%" height="10" align="center" valign="middle"
						...
						<ec:table items="sellOrders" var="map" width="100%" form="queryForm"
							action="${ctx}/orderInputAction!listBuyOrder"
								...
								<tr>
									<td>
										<div id="buttonCRUD"
											...
											valign="middle">
											<display:security urlId="4101104">
											...
											<display:security urlId="4101103">
											...
											<display:security urlId="4101102">
											...
											<display:security urlId="4101101">
											<div id="addBtn" class="btn"
												style="width: 60px; height: 18px; float: right; background: url(${ctx}/images/icon/add.gif) no-repeat; margin: 5px 5px 0; padding: 2px 15px 0 0"
