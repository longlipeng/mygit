	...
	<head>
		...
		<script type="text/javascript">
			...
			function submitForm(method) {
				...
				if(count==1){
						document.EditForm.action="orderImmediatelyCreditAction!"+method;
	    	 ...
	    	 function viewDetail(orderId,batchNo,flag){
	    		 ...
	    		 if(flag==1){
	    				TimeOperation(orderId,
	    						'recharge',
	    						"${ctx}/batchfile/getOrderState.action",
	    						"${ctx}/batchfile/getRechargeBatchDetail.action?order_id="+orderId+"&batch_id="+batchNo,
	    						'充值中,请耐心等候...','5s');
	    			} else if (flag==2){
	    				
	    				  location.href="${ctx}/batchfile/getRechargeBatchDetail.action?order_id="+orderId+"&batch_id="+batchNo;  
	    			}
	    		
	    	  	/* window.showModalDialog('${ctx}/batchfile/rechargeView.action?order_id='+orderId+'&batch_id='+batchNo+'&flag='+flag, '_ blank', 'center:yes;dialogHeight:400px;dialogWidth:600px;resizable:no'); */
	    	 }
		</script>
	</head>
	<body>
		...
		<div class="TitleHref">
			<span>订单管理-->订单立即充值</span>
		</div>
		
		<div id="ContainBox">
		<s:form id="queryForm" name="queryForm"
								action="orderImmediatelyCreditAction!list">
				...
				<tr>
					<td width="98%" height="10" align="center" valign="middle"
						...
						<ec:table items="sellOrders" var="map" width="100%" form="queryForm"
							action="${ctx}/orderImmediatelyCreditAction!list"
							imagePath="${ctx}/images/extremecomponents/*.gif" view="html"
							retrieveRowsCallback="limit" tableId="sellOrder" autoIncludeParameters="false">
							<ec:exportXls fileName="CreditOrderList.xls" tooltip="导出Excel" />
							<ec:row ondblclick="javaScript:viewOrder('${map.orderId}','${map.orderType}');">
								...
								<ec:column property="entityId"   title="客户编号" width="10%" />
								<ec:column property="firstEntityName"   title="客户" width="10%" />
								<ec:column property="processEnntityName"   title="营销机构" width="10%" />
								...
								<ec:column property="orderState"   title="明细" width="6%" viewsAllowed="html">
