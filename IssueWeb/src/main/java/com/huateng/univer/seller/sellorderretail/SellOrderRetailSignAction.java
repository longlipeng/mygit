 ...
 *
 */
public class SellOrderRetailSignAction extends OrderBaseAction{
	
	private Logger logger = Logger.getLogger(SellOrderRetailSignAction.class);
	/**
	 *
	 */
	...
	protected void initOrderType() {	
		sellOrderDTO.setOrderType(OrderConst.ORDER_TYPE_ORDER_SELL_SELLER_RETAIL_SIGN);
		//查找散户客户
	...
	public void validateInsert(){
		...
		if(this.hasFieldErrors()){
			add();
			error_jsp = "WEB-INF/pages/univer/seller/order/orderinput/sellorderretail/signorder/sellOrderRetailAdd.jsp";
		}
	}
	...
	public String insert(){
	    ...
	    public String addOrderList() throws Exception {
		 try{
	      //  SellOrderInputDTO sellOrderInputDTO = new SellOrderInputDTO();
	        ...
	        customerDTO=(CustomerDTO)sendService(ConstCode.CUSTOMER_SERVICE_VIEW, customerDTO).getDetailvo();	
	        if(null == cardholderDTO){
				cardholderDTO = new CardholderDTO();
	        }
	        cardholderDTO.setEntityId(customerDTO.getEntityId());
	        cardholderDTO.getCustomerDTO().setCustomerName(customerDTO.getCustomerName());
	        if(null!=customerDTO.getDepartmentList() || customerDTO.getDepartmentList().size()>0){
	        	cardholderDTO.setDepartmentId(customerDTO.getDepartmentList().get(0).getDepartmentId());
	        }
	        cardholderDTO.setExternalId(customerDTO.getExternalId());
		...
		public String insertCardholder() throws Exception{
			...
			cardholderDTO.setLoginUserId(this.getUser().getLoginUserId());
			cardholderDTO.setOrderId(sellOrderDTO.getOrderId());
			cardholderDTO.setCardholderState("1");
			cardholderDTO=(CardholderDTO)this.sendService(ConstCode.CARDHOLDER_SERVICE_INSERT_CARDHOLDERDTO, cardholderDTO).getDetailvo();
			...
			for(int i=0;i<Integer.parseInt(num);i++){
				list.add(cardholderDTO.getCardholderId());
			}
			orderListStr=DataChange.listToStringArray(list);
			logger.debug(orderListStr);
		...
		public void  validateUpdate(){
			...
			if(this.hasFieldErrors()){
				edit();
				error_jsp="WEB-INF/pages/univer/seller/order/orderinput/sellorderretail/signorder/sellOrderRetailSignEdit.jsp";
			}
		}
