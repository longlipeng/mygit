Ext.onReady(function() {
	...
	var sumGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=sumrzInfUpYQFe'
		}),
		reader: new Ext.data.JsonReader({
			...
			{name: 'accFlag',mapping: 'accFlag'},//auditStatus,auditId,auditDate,recId,recDate,
	...
	function saStateValAudit(val){
		...
		if(val=='5'){
			return '<font color="green">二次划款审核拒绝</font>';
		}
	}
...
//						amount  = parseFloat(count.get('sumAmt')) + parseFloat(amount);
					...
					showConfirm('你勾选了'+sucRecords.length+'条记录，金额：'+ amount +'.确认划款！',sumGrid,function(bt) {
						if(bt == 'yes') {
						...
						var array = new Array();
							for(var index = 0; index < sucRecords.length; index++) {
								...
								var data = {
										...
										auditStatus: record.get('auditStatus'),//定向委托账号名称
										dirAccount: record.get('dirAccount')//定向委托账号
								};
								array.push(data);
							}
							Ext.Ajax.request({
//							url: 'T80224Action.asp?method=remittance',
							url: 'T80224Action_applyYqZlTwo.asp',
							params: {
								infList: Ext.encode(array),
								txnId: '10012',
								subTxnId: '06'
