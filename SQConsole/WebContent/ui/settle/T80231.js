Ext.onReady(function() {
	...
	var sumGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=sumrzInfUpYQauditSE'
		}),
 ...
 **/
	...
	Number.prototype.add = function(arg) {
			...
			var sucRecords = sumGrid.getSelectionModel().getSelections();
					...
					showConfirm('你<font color="red">审核</font>'+sucRecords.length+'条记录，金额：'+ amount +'.确认拒绝！',sumGrid,function(bt) {
						...
						var array = new Array();
							...
							Ext.Ajax.request({
							url: 'T80224Action_refuseRemTwo.asp',
							params: {
			...
			var sucRecords = sumGrid.getSelectionModel().getSelections();
					...
					showConfirm('你<font color="red">审核</font>'+sucRecords.length+'条记录，金额：'+ amount +'.确认通过！',sumGrid,function(bt) {
						...
						var array = new Array();
							...
							Ext.Ajax.request({
							url: 'T80224Action_remittanceTwo.asp',
							params: {
								infList: Ext.encode(array),
								txnId: '10012',
								subTxnId: '04'
		...
		menuArr.push(queryCondition);   //[0]查询
		menuArr.push('-');				//[1]
		menuArr.push(refuseMenu); 		//[2]通过
		menuArr.push('-');				//[3]
		menuArr.push(sucMenu); 			//[4]拒绝
