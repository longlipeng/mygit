Ext.onReady(function() {
	...
	var sumGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=sumrzInfUpYQFF'
		}),
	...
	var sumColModel = new Ext.grid.ColumnModel([
		...
		{header: '交易流水号',dataIndex: 'sumrzBatch',width:200,align: 'center',hidden:true},
	]);
...
//						amount  = parseFloat(count.get('sumAmt')) + parseFloat(amount);
						amount = accAdd(count.get('sumAmt'), amount);
					}
					showConfirm('你勾选了'+sucRecords.length+'条记录，金额：'+ amount +'.确认回填！',sumGrid,function(bt) {
						if(bt == 'yes') {
						if(sucRecords.length == 0) {
							showErrorMsg("请勾选数据！",sumGrid);
							return;
						}
						showMask("正在提交信息，请稍后....",sumGrid);
						var array = new Array();
							for(var index = 0; index < sucRecords.length; index++) {
								...
								var data = {
										...
										sumrzBatch: record.get('sumrzBatch'),//交易流水号
								};
								array.push(data);
							}
							Ext.Ajax.request({
							url: 'T80701Action_backfill.asp',
							params: {
								infList: Ext.encode(array),
								txnId: '802270',
								subTxnId: '01'
...
//						amount  = parseFloat(count.get('sumAmt')) + parseFloat(amount);
					...
					showConfirm('你勾选了'+sucRecords.length+'条记录，金额：'+ amount +'.确认回填失败！',sumGrid,function(bt) {
						if(bt == 'yes') {
						if(sucRecords.length == 0) {
							showErrorMsg("请勾选数据！",sumGrid);
							return;
						...
						var array = new Array();
							...
							Ext.Ajax.request({
							url: 'T80701Action_failureMenu.asp',
							params: {
								infList: Ext.encode(array),
								txnId: '8022702',
								subTxnId: '01'
...
//						amount  = parseFloat(count.get('sumAmt')) + parseFloat(amount);
					...
					showConfirm('你勾选了'+sucRecords.length+'条记录，金额：'+ amount +'.确认回填成功！',sumGrid,function(bt) {
						if(bt == 'yes') {
						if(sucRecords.length == 0) {
							showErrorMsg("请勾选数据！",sumGrid);
							return;
						...
						var array = new Array();
							...
							Ext.Ajax.request({
							url: 'T80701Action_succeedMenu.asp',
							params: {
								infList: Ext.encode(array),
								txnId: '8022701',
								subTxnId: '01'
		...
		menuArr.push('-');				    //[1]
		menuArr.push(sucMenu);				//[2] 商户回填
		menuArr.push('-');				    //[3]
		menuArr.push(failureMenu);			//[5] 失败
		menuArr.push('-');				    //[6]
		menuArr.push(succeedMenu);			//[7] 成功
