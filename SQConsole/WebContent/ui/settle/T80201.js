var store;
var secondMainPanel;
function missionConsole(dateSettlm,brhId,money){
            var buttons = document.getElementsByName('misBut');
            for(var i = 0;i<buttons.length;i++){
            	buttons[i].disabled = true;
            }
            
            HandleOfBatch.sendReqMsg(dateSettlm,brhId,money,function(ret){
                if(ret == '2'){
                    showSuccessMsg("划拨成功",secondMainPanel);
                }else if(ret == '1'){
                	showErrorMsg("划拨失败",secondMainPanel);
                }else{
                	showErrorMsg("未知结果，请查询后确认",secondMainPanel);
                }
                store.load();
            });
        }
Ext.onReady(function() {
    ...
    function doIt(val,metadata,record,rowIndex){
        var money = record.get('settleAmt');
        var brhId = record.get('brhCode');
        var dateSettlm = record.get('dateSettlm');
        if(val == '0' && money != 0) {
        	return "<button class=btn_2k3 id='"+brhId+"' name='misBut' onclick='javascript:missionConsole("+dateSettlm+","+brhId+","+money+")'>启动任务</button>";
        } else {
        	return "";
        }
    }
    ...
    var cm = new Ext.grid.ColumnModel([
		...
		{header: '操作',dataIndex: 'settleFlag',width: 100,renderer:doIt}
	]);
