Ext.onReady(function() {
	var sm = new Ext.grid.CheckboxSelectionModel({
				...
				listeners : {
					'beforerowselect' : function(sumColModel, rowIndex, keepExisting, record) {
						if ((record.data.saStatus) == '1' || (record.data.saStatus) == '2' || (record.data.saStatus) == '0'  ) {
							return false; // 不能进行选择
						} else {
							return true;
						}
	...
	var sumGridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=sumrzInfUpYQaudit'
		}),
		reader: new Ext.data.JsonReader({
			...
			{name: 'instDate',mapping: 'instDate'},//auditStatus,auditId,auditDate,recId,recDate
			...
			handler: function() {
				if(sucAddForm.getForm().isValid()) {
					...
					sucAddForm.getForm().submit({
						url: 'T80701Action_applyYqZl.asp',
