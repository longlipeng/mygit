Ext.onReady(function() {
	...
	var tblRiskMchtTranLimit = new Ext.data.Store( {
		proxy : new Ext.data.HttpProxy( {
			url : 'gridPanelStoreAction.asp?storeId=tblRiskTermTranLimit'
		}),
	...
	tblRiskMchtTranLimit.load( {
		...
		var ctlTxnInfForm = new Ext.form.FormPanel( {
			...
			items : [
				...
				listeners: {
					'select': function() {
				        T40216.getMchtInfo(this.getValue(),function(ret){
						    var tblMchtBaseInfTmp = Ext.decode(ret.substring(1,ret.length-1));
							Ext.getCmp('mchtNm').setValue(tblMchtBaseInfTmp.mchtNm);
						});
					}
				}*/
			},/* {
				...
				buttons : [{
						...
						handler : function() {
							if (ctlTxnInfForm.getForm().isValid()) {
								...
								{
									url : 'T40216Action.asp?method=add',
									waitMsg : '正在提交，请稍后......',
		...
		var upMenu = {
			...
			for ( var index = 0; index < modifiedRecords.length; index++) {
				...
				var data = {
					...
					cardTypeOld : record.get('cardType')
				};
				array.push(data);
			}
			showProcessMsg('正在保存信息，请稍后......');
			Ext.Ajax.request( {
				url : 'T40216Action.asp?method=update',
				method : 'post',
				params : {
				    riskTermTranList : Ext.encode(array),
					txnId : '40216',
					subTxnId : '02'
				},
						...
						// 如果点击了提示的确定按钮
							if (bt == "yes") {
								Ext.Ajax.requestNeedAuthorise( {
									url : 'T40216Action.asp?method=delete',
									success : function(rsp, opt) {
										var rspObj = Ext.decode(rsp.responseText);
										if (rspObj.success) {
											showSuccessMsg(rspObj.msg, grid);
		...
		var queryForm = new Ext.form.FormPanel( {
			...
			items : [{
				...
				emptyText: '请输入商户编号',
//				methodName: 'getMchtcd',
				methodName: 'getMchtIdNew',
				id: 'idMCHTNO',
				hiddenName: 'mchtNo',
				lazyRender: true,
				width: 300,
				callFunction:function() {
					Ext.getCmp('idtermID2').reset();
					Ext.getCmp('idtermID2').parentP=this.value;
					Ext.getCmp('idtermID2').getStore().reload();
				}
//				listeners: {
//					'change': function() {
//						if(this.getValue() == ""){
//							Ext.getCmp('idtermID2').reset();
//							Ext.getCmp('idtermID2').parentP="";
//							Ext.getCmp('idtermID2').getStore().reload();
//						}
//					},'select':function(){
//						//选择商户编号重置终端的数据，并将商户号作为参数传给终端以便于查出该商户号下的终端信息
...
//					}
//					'change': function() {
////						Ext.getCmp('idtermID2').setValue("");
//						if(queryForm.findById('idMCHTNO').getValue() == ""){
//							Ext.getCmp('idtermID2').reset();
//							Ext.getCmp('idtermID2').parentP="";
//							Ext.getCmp('idtermID2').getStore().reload();
//						}
...
//				}
		...
		tblRiskMchtTranLimit.on('beforeload', function(){
		Ext.apply(this.baseParams, {
            start: 0,		
            mchtNo:queryForm.findById('idMCHTNO').getValue(),
			termID: queryForm.findById('idtermID2').getValue(),
