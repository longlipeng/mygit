Ext.onReady(function() {
	...
	// 待审核的工行终端通道配置信息数据集
		...
		var routColModel = new Ext.grid.ColumnModel( [sm,{
			header : '主键',	
			dataIndex : 'id',
			width : 80,
			hidden: true
					...
					if (grid.getSelectionModel().hasSelection()){
					   ...
					   showConfirm('确认审核通过吗？',grid,function(bt) {
						if(bt == 'yes') {
							...
							Ext.Ajax.request({
								url: 'T11004Action.asp?method=accept',
								params: {
									reserved: selections,
								    //id: rec.get('id'),
									txnId: '11004',
			...
			function refuse(bt,text) {
			   ...
			   for(var i=0 ; i<records.length ; i++){
				...
				if(bt == 'ok') {
					...
					rec = grid.getSelectionModel().getSelected();
					Ext.Ajax.request({
						url: 'T11004Action.asp?method=refuse',
						params: {
						    //id: rec.get('id'),
