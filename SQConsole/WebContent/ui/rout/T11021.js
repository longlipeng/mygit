	...
	// 待审核的交行终端通道配置信息数据集
		var termChannelStore = new Ext.data.Store( {
			proxy : new Ext.data.HttpProxy( {
				url : 'gridPanelStoreAction.asp?storeId=termChannelInf2ToCheck'
			}),
...
//			})
				...
				handler:function() {
					showConfirm('确认审核通过吗？',grid,function(bt) {
						if(bt == 'yes') {
							...
							Ext.Ajax.request({
								url: 'T11021Action.asp?method=accept',
								params: {
								    id: rec.get('id'),
									txnId: '11021',
									subTxnId: '01'
		...
		var refuseMenu = {
			...
			function refuse(bt,text) {
				if(bt == 'ok') {
					...
					rec = grid.getSelectionModel().getSelected();
					Ext.Ajax.request({
						url: 'T11021Action.asp?method=refuse',
						params: {
						    id: rec.get('id'),
							txnId: '11021',
							subTxnId: '02',
		...
		grid.on({
			'dblclick':function(){
				rec = grid.getSelectionModel().getSelected();
				if(rec != null){
				showTermChannel2Detail(agencyRen(rec.get('termins')),	//所属机构
					mchntRen(rec.get('mchtid')),		//商户ID
					cardRen(rec.get('cardType')),		//卡类型
		...
		termChannelStore.on('beforeload', function() {
			Ext.apply(this.baseParams, {
				...
				cardType: queryForm.findById('idcardType').getValue(),
