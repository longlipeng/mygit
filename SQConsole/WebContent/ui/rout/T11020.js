Ext.onReady(function() {
	...
	SelectOptionsDWR.getComboDataWithParameter('AGENCY_ID_BANK',"('0301')",function(ret){	//只查询交行机构
	...
	var termChannelStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=termChannelInf2'
		}),
		reader: new Ext.data.JsonReader({
		   ...
		   {name : 'insmcht2',mapping : 'insmcht2'},
		   {name : 'insterm2',mapping : 'insterm2'},
		...
		function cardRen(val){
			if(val == ''){
				return "";
			}else if(cardTypeStore.query("valueField",val).first() == undefined){
				return val;
			}else{
...
//			})
				...
				handler : function() {
					if (routInfoForm.getForm().isValid()) {
						...
						routInfoForm.getForm().submit({
							url : 'T11020Action.asp?method=add',
							waitMsg : '正在提交，请稍后......',
		...
		var upMenu = {
			...
			for ( var index = 0; index < modifiedRecords.length; index++) {
				...
				var data = {
					...
					insMcht2 : record.get('insmcht2'),
					insTerm2 : record.get('insterm2'),
					lmk : record.get('lmk'),
			...
			Ext.Ajax.request( {
				url : 'T11020Action.asp?method=update',
				method : 'post',
				params : {
						...
						// 如果点击了提示的确定按钮
							if (bt == "yes") {
								Ext.Ajax.requestNeedAuthorise( {
									url : 'T11020Action.asp?method=delete',
									success : function(rsp, opt) {
										var rspObj = Ext.decode(rsp.responseText);
										if (rspObj.success) {
											showSuccessMsg(rspObj.msg, grid);
		...
		termChannelStore.on('beforeload', function() {
			Ext.apply(this.baseParams, {
				...
				insMcht : queryForm.findById('idsearchInsMcht2').getValue(),
				insTerm : queryForm.findById('idsearchInsTerm2').getValue(),
