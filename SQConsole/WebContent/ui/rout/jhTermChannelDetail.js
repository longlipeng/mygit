//终端通道详细信息，在外部用函数封装
function showTermChannel2Detail(termIns,			//所属机构
								mchtId,			//商户ID
								cardType,		//卡类型
								reserve,		//索引值
								creTime,		//创建时间
	...
	var selectedRecModel = new Ext.data.Record.create([
		...
		{name : 'cardType',mapping : 'string'},
		{name : 'insmcht2',mapping : 'string'},
		{name : 'insterm2',mapping : 'string'},
	...
	selectedModel.set('cardType', cardType);			//卡类型
	selectedModel.set('insmcc',allSign(rec.get('insmcc')));		//机构MCC
	selectedModel.set('insmcht',allSign(rec.get('insmcht')));	//核心机构商户号
	selectedModel.set('insterm',allSign(rec.get('insterm')));	//核心机构终端号
	selectedModel.set('insmcht2',allSign(rec.get('insmcht2')));	//交易机构商户号
	selectedModel.set('insterm2',allSign(rec.get('insterm2')));	//交易机构终端号
