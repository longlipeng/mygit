...
//						}
		...
		var upMenu = {
			...
			for ( var index = 0; index < modifiedRecords.length; index++) {
				...
				var minAmt=record.get('minAmt').split('.')[0]+(record.get('minAmt').split('.')[1]==null?"00":record.get('minAmt').split('.')[1]);
				var maxAmt=record.get('maxAmt').split('.')[0]+(record.get('maxAmt').split('.')[1]==null?"00":record.get('maxAmt').split('.')[1]);
				var data = {
					id : record.get('id'),
