...
Ext.onReady(function() {// 可选机构数据集
	...
	var gridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=transhangshenhejie'
		}),
	...
	var brhColModel = new Ext.grid.ColumnModel([
		...
		{header: '商户编号',dataIndex: 'mchtno',width: 120},
		{header: '商户名称',dataIndex: 'mchtnm',width: 220},
		{header: '交易流水号',dataIndex: 'flownum',width: 120},
		{header: '交易日期',dataIndex: 'transdate',width: 100},	
		{header: '挂账标志',dataIndex: 'hangflag',width: 80,renderer:statue},
		{header: '挂账日期',dataIndex: 'hangdate',width: 80},
		{header: '解挂日期',dataIndex: 'relievedate',width: 80}	,
		{header: '挂账原因',dataIndex: 'remark',width: 200}//,
//		{header: '系统流水号',dataIndex: 'txnssn',width: 200,hidden:true}	
	]);
		...
		handler:function() {
			showConfirm('确认审核通过吗？',grid,function(bt) {
				if(bt == 'yes') {
					...
					showMask("正在提交审核信息，请稍后......",grid);
					rec = grid.getSelectionModel().getSelected();
					Ext.Ajax.request({
						url: 'T10030Action.asp?method=accept',
						timeout : 600000,
						params: {
							mchtno:rec.get('mchtno'),
							transdate:rec.get('transdate'),
							txnssn: rec.get('txnssn'),
					        flownum: rec.get('flownum'),
							txnId: '10030',
							subTxnId: '01'
						},
	...
	function refuse(bt,text) {
		if(bt == 'ok') {
			...
			rec = grid.getSelectionModel().getSelected();
			Ext.Ajax.request({
				url: 'T10030Action.asp?method=refuse',
				timeout : 600000,
				params: {
					mchtno:rec.get('mchtno'),
					transdate:rec.get('transdate'),
