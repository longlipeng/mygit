Ext.onReady(function(){
	...
	var manuStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=changeAccInfTmp',
			timeout: 120000
		}),
...
// 		{header: '已完成金钱',dataIndex:'confirmAccount'},
	...
	var changeroutInfoForm = new Ext.form.FormPanel( {// 路由添加表单
		frame : true,
		autoHeight : true,
		width : 600,
		labelWidth : 90,
	...
	var queryForm = new Ext.form.FormPanel( {
		...
		items : [/*{
           ...
           width: 300
			...
			store: new Ext.data.ArrayStore({
				fields: ['valueField','displayField'],
				data: [['0','正常'],['1','新增未审核'],['2','修改未审核'],['3','删除未审核'],['4','已删除']],
				reader: new Ext.data.ArrayReader()
			})
		},{
			xtype: 'panel',
	...
	var Rdownload = {
			...
			iconCls: 'download',
			handler:function() {
				showMask("正在为您准备报表，请稍后....",manuGrid);
				Ext.Ajax.requestNeedAuthorise({
					url: 'T10039_DwnAction.asp',
					params: {//查询的参数
					mchtNo: queryForm.getForm().findField('idmchtNoQ').getValue(),
		            termId: queryForm.getForm().findField('termIdQ').getValue(),
		            enterTpQ: queryForm.getForm().findField('enterTpQ').getValue(),
