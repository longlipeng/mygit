...
Ext.onReady(function(){
 ...
 var manuStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=manualReturnWeih'
		}),
...
//            termId: queryForm.getForm().findField('termId').getValue(),
            termId: queryForm.getForm().findField('idtermNo').getValue(),
            ...
            saState: queryForm.getForm().findField('saState').getValue(),
	...
	var upMenu = {
			...
			handler: function() {
				...
				for(var index = 0; index < modifiedRecords.length; index++) {
					...
					var amtTranS=record.get('amtTrans');
                	var amtReturn= record.get('amtReturn');
                	var saState= record.get('saState')
                    if(Number(amtReturn)==0){
                    ...
                    if(saState == '5' || saState == '1' ){
                    	showErrorMsg("已退货成功不得修改金额",manuGrid);
                        return;
                   }
	...
	var queryForm = new Ext.form.FormPanel( {
		...
		items : [{
			...
			callFunction: function() {
				Ext.getCmp('idtermNo').reset();
				Ext.getCmp('idtermNo').parentP=this.value;
				Ext.getCmp('idtermNo').getStore().reload();
			}
		},{
               xtype: 'dynamicCombo',
               fieldLabel: '终端号',
...
//			allowBlank: false,
			...
			store: new Ext.data.ArrayStore({
				...
				data: [['0','新增待审核'],['1','退货成功'],['2','修改待审核'],['3','删除待审核'],['4','审核拒绝'],['5','后台退货成功，前台处理错误']],
...
//			width: 300
	...
	var cityCodeForm = new Ext.form.FormPanel({
		...
		items: [{
			...
			width:200,
			callFunction: function() {
				Ext.getCmp('termId1').reset();
				Ext.getCmp('termId1').parentP=this.value;
				Ext.getCmp('termId1').getStore().reload();
			}
		},{
               xtype: 'dynamicCombo',
               fieldLabel: '终端号',
