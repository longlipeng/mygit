...
Ext.onReady(function() {// 可选机构数据集
	...
	var gridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=mchthang'
		}),
	...
	var brhColModel = new Ext.grid.ColumnModel([
		...
		{header: '商户编号',dataIndex: 'mchtno',width: 150},
	...
	var refuseMenu = {
		...
		disabled: true,
		handler:function() {
			showConfirm('确认解挂吗？',grid,function(bt) {
				if(bt == 'yes') {
					showInputMsg('提示','请输入解挂原因',true,refuse);
				}
			});
		}
	};
	...
	function refuse(bt,text) {
		if(bt == 'ok') {
			if(getLength(text) > 60) {
				alert('解挂原因最多可以输入30个汉字或60个字母、数字');
				showInputMsg('提示','请输入解挂原因',true,refuse);
				return;
			}
			showProcessMsg('正在提交解挂信息，请稍后......');
