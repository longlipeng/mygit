...
var acceptMenu = {
		...
		disabled: true,
		handler:function() {
			showConfirm('确认审核通过吗？',gridPanel,function(bt) {
				if(bt == 'yes') {
					showProcessMsg('正在提交审核信息，请稍后......');
					rec = gridPanel.getSelectionModel().getSelected();
					Ext.Ajax.request({
						url: 'T20706Action.asp?method=accept',
						params: {
							discCd: rec.get('discCd'),
							txnId: '20706',
							subTxnId: '01'
						},
	...
	var refuseMenu = {
		...
		disabled: true,
		handler:function() {
			showConfirm('确认拒绝吗？',gridPanel,function(bt) {
				if(bt == 'yes') {
	...
	function refuse(bt,text) {
		if(bt == 'ok') {
			...
			if(getLength(text) > 60) {
				alert('拒绝原因最多可以输入30个汉字或60个字母、字符或数字');
				showInputMsg('提示','请输入拒绝原因',true,refuse);
				return;
			}
			showProcessMsg('正在提交审核信息，请稍后......');
			rec = gridPanel.getSelectionModel().getSelected();
			Ext.Ajax.request({
				url: 'T20706Action.asp?method=refuse',
				params: {
					discCd: rec.get('discCd'),
					txnId: '20706',
					subTxnId: '03',
	...
	var gridStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=tblInfDiscCdShenHe'
		}),
		reader: new Ext.data.JsonReader({
			root: 'data',
			totalProperty: 'totalCount'
	...
	var gridColumnModel = new Ext.grid.ColumnModel([
		...
		{header: '申请人',dataIndex: 'recCreUserId',sortable: true},
		{header: '申请时间',dataIndex: 'recCrtTs',sortable: true},
	...
	gridStore.on('beforeload', function(){
		Ext.apply(this.baseParams, {
			start: 0,
			discCd: queryForm.getForm().findField('discCd').getValue(),
			discNm: queryForm.getForm().findField('discNm').getValue(),
