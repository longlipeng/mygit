		...
		callback: function(records, options, success){
			if(success){
				...
				if(setRpt=='1'){
					...
					Ext.getCmp('settleAcct').enble();
                    Ext.getCmp('settleAcctNm').enble();;
				}
				   ...
				   //初始化省、市、县信息
                      SelectOptionsDWR.getComboDataWithParameter('CITIES',mchntForm.getForm().findField('idprovince').getValue(),
							function(ret){
								...
								//Ext.getCmp('idcityUpd').setValue(rec.get('valueField'));
				...
				,{
		        	...
		        	layout: 'form',
	       			items: [{
						...
						labelStyle: 'padding-left: 5px',
						fieldLabel: '注册资金(万元)*',
						regex: /^(([0-9]+\.[0-9]*[0-9][0-9]*)|([0-9]*[1-9][0-9]*\.[0-9]+)|([0-9]*[0-9][0-9]*))$/,
	       			...
	       			items: [{
    					...
    					'select': function() {
							  ...
							  if(flag=='86'){//中国国籍 则证件类型为身份证
							  	...
							  	mchntForm.getForm().findField('artifCertifTp').setValue("02");
                ...
                items: [{
		        	...
		        	hidden:true,
	       			items: [{
						...
						fieldLabel: '商户结算账户开户行名称*',
						anchor: '90%',
						//allowBlank: false,
						allowBlank: true,
						id: 'settleBankNmI',
						hiddenName: 'settleBankNm'
		        	}]
				},{
	        		//columnWidth: .5,
					columnWidth: .33,
