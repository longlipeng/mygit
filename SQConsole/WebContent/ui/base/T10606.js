Ext.onReady(function() {
	...
	var txnStore = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: 'gridPanelStoreAction.asp?storeId=attendanceInfo'
		}),
...
//			{name: 'mchtnm',mapping: 'mchtnm'},
	...
	var txnColModel = new Ext.grid.ColumnModel([
		{header: '签到行编号',dataIndex: 'instid',width: 100,hidden:false},
		{header: '签到行名称',dataIndex: 'instname',width: 120,hidden:false},
		{header: '商户号',dataIndex: 'mchtid',width: 100,hidden:false},
//		{header: '商户',dataIndex: 'mchtnm',width: 330},
		{header: '终端号',dataIndex: 'termid',width: 80},
		{header: '签到状态',dataIndex: 'signedflag',width: 80,renderer:sign},
		{header: '签到时间',dataIndex: 'signtime',width: 120,renderer:formatTs},
	...
	var upMenu = {
		...
		handler:function() {
			if(grid.getSelectionModel().hasSelection()) {
				...
				showConfirm('确定要签到吗？签到终端号：' + termid,grid,function(bt) {
					//如果点击了提示的确定按钮
					if(bt == "yes") {
						Ext.Ajax.requestNeedAuthorise({
							url: 'T10600Action.asp?method=update',
							success: function(rsp,opt) {
								var rspObj = Ext.decode(rsp.responseText);
								if(rspObj.success) {
									showSuccessMsg("签到成功",grid);
...
//					if(this.getValue() == ""){
//						Ext.getCmp('idTERM_ID').reset();
//						Ext.getCmp('idTERM_ID').parentP="";
//						Ext.getCmp('idTERM_ID').getStore().reload();
//					}
//				},'select': function() {
//						//选择商户号重置终端的数据，并将商户号作为参数传给终端以便于查出该商户号下的终端信息
//						Ext.getCmp('idTERM_ID').reset();
//						Ext.getCmp('idTERM_ID').parentP=this.value;
//						Ext.getCmp('idTERM_ID').getStore().reload();
//					}
