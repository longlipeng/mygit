...
function disableButton(frm)
	{
		var ObjectSum = frm.elements.length;
		...
		for (var i = 0 ; i <= ObjectSum; i++)
		{
			...
			if (objectType == "button"||objectType =="reset")
			{
...
/*submitdate(entityName,datename)拼接日期区间sql*/
function submitdate(entityName,datename){
	var startdate=searchform.START_DATE.value;
	var enddate=searchform.END_DATE.value;
	if(startdate==""){
		startdate="10000101";
	}
	if(enddate==""){
	...
	searchform.elements[entityName+"/_where"].value=datename+" between '"+startdate+"' and '"+enddate+"'";
}

/*chedate(stdate, enddate)判断开始时间和结束时间的大小关系*/
function chedate(stdate, enddate){
    if(stdate.value==""||enddate.value=="") return true;
    if(enddate.value<stdate.value){
	 	 alert("输入域："+enddate.eos_displayname+" 应该大于等于："+stdate.eos_displayname+ " ！");
	     enddate.focus();
	     return false;
    }
    return true;
...
//显示帮助信息
	...
	var helpX,helpY;
	var win;
	var diaWidth=675,diaHeight=385;
	...
	function startRequest(menuid){
		createXMLHttpRequest();
		xmlHttp.onreadystatechange = handleStateChange;
		xmlHttp.open("GET","com_menuhelp.pr.P_MenuHelp_QueryMessage.do?HELP_INFO/MENUID="+menuid,true);
		xmlHttp.send(null);		
	}
		...
		if(xmlHttp.readyState==4){
			try{
				win.focus();
			  	...
			  	win =window.showModelessDialog('_blank',null,"dialogWidth:"+ diaWidth + "px;" + "dialogHeight:"+diaHeight + "px;dialogLeft:" 
									+ (helpX-diaWidth) + "px;dialogTop:" + helpY 
									+ "directories:no;help:no;status:no;resizable:no;scrollbars:no;");
	...
	function showHelpButton1(){
		if(showHelpButton){
			document.write("<div style=\"position: absolute\">");
		}
	}
	
	function showHelpButton2(menid){
		if(showHelpButton){
			document.write("</div><div  style=\"text-align: right\"><SPAN  onclick=\"showHelp('"+menid+"');\" style=\"CURSOR: hand\">帮助</SPAN></div>");
		}
	}