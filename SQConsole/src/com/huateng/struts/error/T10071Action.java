...
public class T10071Action extends BaseAction {
	...
	protected String subExecute() throws Exception {
		try {
			 if("complete".equals(getMethod())) {
				rspCode = complete();
			 }
		} catch (Exception e) {
			log("操作员编号：" + operator.getOprId()+ "，操作手工预授权完成失败，失败原因为："+e.getMessage());
		}
		return rspCode;
	}
	...
	private String complete() {
		...
		try {
			result = preAuth();
			System.out.println("手工预授权完成响应结果："+result);
			
			if("00".equals(result)){
				log("操作员编号：" + operator.getOprId()+"传输成功");
			}else if(!"00".equals(result)){
				log("手工预授权完成失败");
				return "手工预授权完成失败：" + result;
			}else{
				log("操作员编号：" + operator.getOprId() + "传输失败");
				return "发送手工预授权完成的报文失败";
			}
		...
		return Constants.SUCCESS_CODE;//手工预授权完成成功
	}
	...
	private String preAuth() throws IOException {
		...
		//组合成字符串
		StringBuffer ss = new StringBuffer("1091");//手工预授权完成代码
