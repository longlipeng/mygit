...
public class T11004Action extends BaseAction {
	...
	protected String subExecute() throws Exception {
		...
		for(int i=0 ; i<this.getReserved().split(";").length ; i++){
			TblTermChannelInf tblTermChannelInf = t11015BO.get(this.getReserved().split(";")[i]);
			if(this.checkOperator(tblTermChannelInf.getModiOprId()))
				return "存在提交人和当前审核人是同一人的记录，请重新选择！";
		}
		
		for(int i=0 ; i<this.getReserved().split(";").length ; i++){
			id = this.getReserved().split(";")[i];
