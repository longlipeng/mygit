	...
	public CardManagementDTO queryCard(CardManagementDTO dto)
		...
		try {
			...
			dto.setCardValidityPeriod(DateUtil.formatStringDate(accPackageDTO
					.getValideTime()));
			dto.setCardActDate(DateUtil.formatStringDate(accPackageDTO
					.getActiveTime()));
			dto.setActive(accPackageDTO.getActiveState());
			if(accPackageDTO.getLockState().equals("2")){
				 dto.setLock("1");
			}else{
			...
			dto.setPinErrTime(accPackageDTO.getOtherData());
			dto.setFreezeStat(accPackageDTO.getFreezesState());
			// 账户数量
		...
		try {
			...
			//for (int i = 0; i < dto.getAccountList().size(); i++) {
				...
				if (!"00".equals(rspCode)) {
					...
					throw new BizServiceException(rspCode);
				}
			//}
		} catch (BizServiceException e) {
			this.logger.error(e.getMessage());
			...
			e.printStackTrace();
			this.logger.error(e.getMessage());
			throw new BizServiceException("单张卡充值失败！");
		}
		return rspCode;
	}
