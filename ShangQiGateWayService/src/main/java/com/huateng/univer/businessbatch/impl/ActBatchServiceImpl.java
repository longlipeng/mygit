...
public class ActBatchServiceImpl extends BasicBatchServiceImpl {
	...
	public void commMessage(BatchFileDTO batchFileDTO)
		...
		try {
			...
			accPackageDTO.setCvv2( batchFileDTO.getCvn2());
			...
			logger.debug("激活返回值："+accPackageDTO.getRespCode());
			if(!accPackageDTO.getRespCode().equals("00")){
				if(accPackageDTO.getRespCode().equals("74")){
				
					throw new BizServiceException("74");
				}else{
					throw new BizServiceException("卡激活错，错误码：【"+accPackageDTO.getRespCode()+"】");
				}
