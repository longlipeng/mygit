...
public class FtpUtil {
    ...
    public static boolean uploadFile(String ip, int port, String userName, String password, String path,
        ...
        try {
            ...
            if (!cd) {
                logger.info(path + " 目录不存在, 生成目录");
                
                boolean mkdirSuccess = ftp.makeDirectory(path);
                if(mkdirSuccess){
                    logger.info(path + " 目录生成成功");
                    ftp.changeWorkingDirectory(path);   
                }else{
                    logger.info(path + " 目录生成失败");
                }
     ...
     * ftp读取文件返回字节数组列表
     */
    public static List<byte[]> downFileStream111(String url, int port, String username, String password,
    ...
    public static List<byte[]> downFileStream(String url, int port, String username, String password,
        ...
        try {
            ...
            for (FTPFile ff : fs) {
                if (ff.getName().equals(fileName)) {
                    ...
                    try {
                        tempResFile = File.createTempFile("tempResFile", ".txt");
    ...
    public static boolean ftpPutFile(String text, String fileName, String ip, Integer port, String userName,
            String password, String uploadFilePath) throws Exception {
        ...
        try {
            ...
            connectFlag = FtpUtil.uploadFile(ip, port, userName, password, uploadFilePath, fileName, fis);
