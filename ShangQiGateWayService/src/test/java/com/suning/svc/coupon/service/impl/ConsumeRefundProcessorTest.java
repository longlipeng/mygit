...
public class ConsumeRefundProcessorTest {

    /**
     * Test method for {@link com.suning.svc.coupon.service.BatchTradeProcessTemplate#batchProcessTrade()}.
     */
    @Autowired
    @Qualifier("consumeRefundProcessor")
    TradeProcessor processor;
    
    @Test
    @DbScriptSetup(value="/dbunit/consumeRefundProcessor.sql", datasource="dataSource")
    @DatabaseSetup(type = DatabaseOperation.INSERT, value = { "/dbunit/consumeRefundProcessor.xml" })
    @ExpectedDatabase(value="/dbunit/consumeRefundOrder.xml", assertionMode=DatabaseAssertionMode.NON_STRICT)
