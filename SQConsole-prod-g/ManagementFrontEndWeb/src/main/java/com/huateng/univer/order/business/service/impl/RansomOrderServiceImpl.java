...
public class RansomOrderServiceImpl implements RansomOrderService {
	...
	@Override
	public void modifyOrigCardListCardStateCallBack(
			SellOrderOrigCardListDTO sellOrderOrigCardListDTO)
			throws BizServiceException {
		// 页面是否修改列表
		String[] origcardListIds = sellOrderOrigCardListDTO
		...
		if (origcardListIds != null && origcardListIds.length > 0) {
			...
			for (String origcardListId : origcardListIds) {
				SellOrderOrigCardList origCard = new SellOrderOrigCardList();
				origCard.setOrigcardListId(origcardListId);
				origCard.setCallBack(sellOrderOrigCardListDTO.getCallBack());
				origcardList.add(origCard);
			}
			try {
				baseDAO.batchUpdate("TB_SELL_ORDER_ORIGCARD_LIST",
	...
	public void submitOrder(SellOrderDTO sellOrderDTO)
		...
		try {
			...
			Map<String, String> OrigCardStateMap = new HashMap<String, String>();
			for(SellOrderOrigCardListDTO sellOrderOrigCardListDTO : list){
				OrigCardStateMap.put(sellOrderOrigCardListDTO.getCardNo(), sellOrderOrigCardListDTO.getCardSate());
			}
			...
			if (null != batchNo && !"".equals(batchNo)) {
				...
				//原售卡订单全部卡信息
				List<SellOrderCardList> origSellOrderCardLists = sellOrderCardListDAO.getSellOrderCardListByRansomOrderId(sellOrderDTO);
				
				boolean hasNotActive = false;
				for (SellOrderCardList SOCList : origSellOrderCardLists) {
					//查询卡激活状态
//					AccPackageDTO accPackageDTO = new AccPackageDTO();
//					accPackageDTO.setTxnCode("S000");
//					accPackageDTO.setCardNo(SOCList.getCardNo());
//					accPackageDTO = java2ACCBusinessService.queryCard(accPackageDTO);
//					if(accPackageDTO.getActiveState().equals("0")){//卡未激活
//						hasNotActive = true;
//					}
					if(OrigCardStateMap.get(SOCList.getCardNo()).equals("2")){//卡入库
						hasNotActive = true;
					}
				...
				//修改原售卡订单状态
				if(hasNotActive){//卡入库
					SellOrder order = sellOrderDAO.selectByPrimaryKey(origSellOrderCardLists.get(0).getOrderId());
					if(order.getOrderState().equals("32")){//配送成功
						SellOrder neworder = new SellOrder();
						neworder.setOrderId(order.getOrderId());
						neworder.setOrderState(OrderConst.ORDER_STATE_ORDER_RANSOM);//原订单赎回
						sellOrderDAO.updateByPrimaryKeySelective(neworder);
					}
				}
				...
				//修改原关联卡的信息为无效
				updateOrderCardListBySellOrderId(list,"0",origSellOrderCardLists.get(0).getOrderId());
	...
	//按原售卡订单去掉持卡人与卡的绑定
	private void updateOrderCardListBySellOrderId(List<SellOrderOrigCardListDTO> list,String dataStat,String origSellorderId) throws BizServiceException {
		if(list!=null&&list.size()!=0){
			List<SellOrderCardList> sellOrderCardLists_temp = new ArrayList<SellOrderCardList>();
			for(int i =0 ;i<list.size();i++){
				//20160530修改为按订单号和卡号修改状态
//				SellOrderCardList socl = new SellOrderCardList();
//				socl.setCardNo(list.get(i).getCardNo());
//				socl.setDataState(dataStat);
//				commonsDAO.updateCardList(socl);
				SellOrderCardListExample example = new SellOrderCardListExample();
				example.createCriteria().andOrderIdEqualTo(origSellorderId)
										.andCardNoEqualTo(list.get(i).getCardNo())
										.andDataStateEqualTo("1");
				SellOrderCardList SellOCList = new SellOrderCardList();
				SellOCList.setDataState("0");
				sellOrderCardListDAO.updateByExampleSelective(SellOCList, example);
				
//				SellOrderOrigCardListDTO sellOrderOrigCardListDTO=new SellOrderOrigCardListDTO();
//				sellOrderOrigCardListDTO=cardholderDAO.selectCardStateByCardNo(list.get(i).getCardNo());
				//判断卡是否是入库
				if(list.get(i).getCardSate().equals("2")){
					SellOrderCardList sellOrderCardList_temp = new SellOrderCardList();
					sellOrderCardList_temp.setCardNo(list.get(i).getCardNo());
			...
			try {
				if(sellOrderCardLists_temp.size()>0){
				...
				e.printStackTrace();
				throw new BizServiceException("删除人卡关联失败！");
			}
		}
	}
//	//去掉持卡人与卡的绑定
//	private void updateOrderCardList(List<SellOrderOrigCardListDTO> list,String dataStat) throws BizServiceException {
//		if(list!=null&&list.size()!=0){
//			List<SellOrderCardList> sellOrderCardLists_temp = new ArrayList<SellOrderCardList>();
//			for(int i =0 ;i<list.size();i++){
//				SellOrderCardList socl = new SellOrderCardList();
...
//	}
	...
	@Override
	public List<SellOrderOrigCardListDTO> ready(SellOrderDTO sellOrderDTO)
			throws BizServiceException {
		// FIXME 准备卡片语句未写
		try {
                ...
                for(int i = 0; i < size; i++) {
                    ...
                    while (it.hasNext()) { 
                        ...
                        if(RansomOrderConst.ACC_CARD_INFO_PRODUCT_ID.equals(key)) {
                        	Product product = productDAO.selectByPrimaryKey(String.valueOf(value));
						...
						if (RansomOrderConst.ACC_CARD_INFO_CARDHOLDER_ID.equals(key)) {
							if (null == value || "".equals(value)) {
								...
								Cardholder cardholder = cardholderDAO.selectByPrimaryKey(hId);
								if (cardholder != null) {
									newMap.put(RansomOrderConst.ACC_CARD_INFO_CARDHOLDER_NAME,
											cardholder.getFirstName()); // 个人持卡人
								} else {
									CompanyInfoKey infoKey = new CompanyInfoKey();
									infoKey.setRelationNo(hId);
									infoKey.setRelationType("01");
									CompanyInfo info = companyInfoDAO.selectByPrimaryKey(infoKey);// 企业持卡人
									newMap.put(RansomOrderConst.ACC_CARD_INFO_CARDHOLDER_NAME, info.getCompanyName()); // 企业持卡人
								}
							}
						}
    ...
    public SellOrderOrigCardListDTO insertCheckCardNew (
        ...
        //查询卡号是否是换卡时销售的卡
        List<String> cardholderList = orderBaseQueryBO
                .getOrigCardChennlList(sellOrderOrigCardListQueryDTO.getCardNo());
        if(cardholderList.size()>0){
        	//如果卡片是在换卡订单时销售的卡，卡片不准入库
        	 origCard.setCardSate(OrderConst.RANSON_ORIG_CARD_STAT_DESTORY);
             map.put("cardState", "验收通过-只允许注销");
        }else{
