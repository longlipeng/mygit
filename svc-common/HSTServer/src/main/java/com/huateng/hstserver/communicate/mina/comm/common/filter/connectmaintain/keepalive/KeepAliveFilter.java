...
public class KeepAliveFilter extends IoFilterAdapter {
	...
	protected final AttributeKey WAITING_FOR_RESPONSE = new AttributeKey(
            getClass(), "waitingForResponse");
	protected final AttributeKey IGNORE_READER_IDLE_ONCE = new AttributeKey(
            getClass(), "ignoreReaderIdleOnce");
	
	protected final AttributeKey RESEND_COUNT = new AttributeKey(
            getClass(), "resendCount");
    ...
    private volatile int resendTimes;
     ...
     * <ul>
     * <li><tt>interestedIdleStatus</tt> - {@link IdleStatus#WRITER_IDLE}</li>
     * <li><tt>policy</tt> = {@link KeepAliveRequestTimeoutHandler#NOOP}</li>
     * <li><tt>keepAliveRequestInterval</tt> - 60 (seconds)</li>
     * <li><tt>keepAliveRequestTimeout</tt> - 30 (seconds)</li>
     * <li><tt>keepAliveResendTimes</tt> - 1 (time)</li>
     * </ul>
     */
    public KeepAliveFilter(KeepAliveMessageFactory messageFactory) {
        this(messageFactory, IdleStatus.WRITER_IDLE, KeepAliveRequestTimeoutHandler.NOOP);
    }
    ...
    public KeepAliveFilter(
    	...
    	this(messageFactory, interestedIdleStatus, policy, keepAliveRequestInterval, keepAliveRequestTimeout, 1);
    	/*
    ...
    public KeepAliveFilter(
            KeepAliveMessageFactory messageFactory,
            IdleStatus interestedIdleStatus, KeepAliveRequestTimeoutHandler policy,
            int keepAliveRequestInterval, int keepAliveRequestTimeout, int keepAliveResendTimes) {
        if (messageFactory == null) {
            throw new NullPointerException("messageFactory");
        }
        if (interestedIdleStatus == null) {
        ...
        setRequestInterval(keepAliveRequestInterval);
        setRequestTimeout(keepAliveRequestTimeout);
        setResendTimes(keepAliveResendTimes);
    }
    ...
    public void sessionIdle(
        ...
        if (status == interestedIdleStatus) {
        	//add for resend request
        	Integer resendCnt = (Integer)session.getAttribute(RESEND_COUNT);
            if (resendTimes > 0 && (!session.containsAttribute(WAITING_FOR_RESPONSE) || resendCnt == null || resendCnt < resendTimes)) {
  ...
  //add for resend request
    protected boolean processResendTimes(IoSession session)
    {
    	if (resendTimes <= 0)
    	{
    		return false;
    	...
    	Integer resendCnt = null;
		if ((resendCnt = (Integer)session.getAttribute(RESEND_COUNT)) !=  null)
		{
			resendCnt ++;
			session.setAttribute(RESEND_COUNT, resendCnt);
			
			if (resendCnt <= resendTimes)
			{
				return true;
			}
		}
    ...
    protected void handlePingTimeout(IoSession session) throws Exception {
    	//add for resend request
    	if (processResendTimes(session))
    	{
    		return;
    	}
    ...
    protected void markStatus(IoSession session) {
    ...
    protected void resetStatus(IoSession session) {
    ...
    protected boolean isKeepAliveMessage(IoSession session, Object message) {
