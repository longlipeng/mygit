...
public class BizAppObjectProcessor implements IAppObjectProcessor{
...
static final String sep = "\r\n";
	...
	public Object msg2obj(byte[] messageBytes) throws Exception{
		...
		if(bodyLen>0){
			...
			logger.info("bodyByteLen:"+bodyByte.length);
			logger.info("bodyLen-10:"+(bodyLen-10));
			logger.info("messageByteslen");
	...
	public byte[] obj2msg(Object messageObject) throws Exception{
		...
		if(null!=message){
			...
			if(message.getPackageNo() != null && !message.getPackageNo().equals("")){
		        ...
		        //拼接报文头
		        byte[] msgTop = this.combineMsgTop(message.getServiceName(),msg,commMessage.getDomanId());
		        //拼接报文
	...
	public String combine(String value){
		...
		if(null==value || "".equals(value)){
			...
			msg+=StringUtils.fillString(String.valueOf(value.getBytes().length),3)+value;
		} 
		return msg;
	}
	...
	//组装报文头
	public byte[] combineMsgTop(String serviceName,String message,String domanid){		
		/**
		 * 报文域总长度
		 * 正常业务类报文，包含10位报文域长度
		 */
		int msgLen = 10;
		byte[] msgTotalLen=StringUtils.StringTobyte(message.getBytes().length + msgLen);
...
//		   biz.combineMsgTop("vCardBatInq", "12345672345678909876543234567898765432345678987654345678765456787456785456786543456785434567865434567865432345678");
