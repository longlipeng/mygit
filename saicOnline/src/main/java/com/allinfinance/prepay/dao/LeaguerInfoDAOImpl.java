...
import com.allinfinance.prepay.processor.ipos.SyncP024Processor;
...
//			leaguerInfo.setLeaguerId(leaguerInfoList.get(i).getLeaguerId());
	...
	@Override
	public List<LeaguerInfo> selectLeaguerInfoByUserIdandUserType(LeaguerInfo leaguerInfo)
			throws BizServiceException {
		try {
			LeaguerInfoExample ex=new LeaguerInfoExample();
			ex.createCriteria().andLeaguerIdEqualTo(leaguerInfo.getLeaguerId())
				.andCusTypeEqualTo(leaguerInfo.getCusType());
			List<LeaguerInfo> info=leaguerInfoMapper.selectByExample(ex);
			return info;
		} catch (Exception e) {
			// TODO: handle exception
			logger.error(e.getMessage());
