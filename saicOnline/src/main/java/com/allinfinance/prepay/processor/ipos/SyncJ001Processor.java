...
import com.allinfinance.prepay.message.MessageSyncJ001Req;
import com.allinfinance.prepay.message.MessageSyncJ001Resp;
...
import com.allinfinance.prepay.utils.CommonUtil;
...
@Service
public class SyncJ001Processor implements IProcessor {
	private static Logger logger = Logger.getLogger(SyncJ001Processor.class);
	// @Autowired
	...
	public BasicMessage process(BasicMessage req) throws Exception {
		MessageSyncJ001Req reqData = (MessageSyncJ001Req) req;
		MessageSyncJ001Resp resp = (MessageSyncJ001Resp) reqData.createResp();
		//屏蔽车享家交易
		if(StringUtils.isNotEmpty(reqData.getTXN_CODE())){
			resp.setRESP_CODE(ResponseCode.INVALID_TRANSACTION);
			logger.error("屏蔽交易！["+reqData.getRESPSEQNO()+"]");
			return resp;
		}
		try {
			if(XmlUtil.cardMap.containsKey(reqData.getCARD_NO())){
				resp.setRESP_CODE(ResponseCode.SUBMIT_DUPLICATE);
				logger.error("重复提交！["+reqData.getRESPSEQNO()+"]");
				return resp;
			}
			...
			if (reqData.getCUSTOMER_TYPE() != null
					&& !reqData.getCUSTOMER_TYPE().equals("")) {
				if (!reqData.getCUSTOMER_TYPE().equals("0")) {
			...
			if (reqData.getCHANNEL() != null
					&& !reqData.getCHANNEL().trim().equals("")) {
				channel = reqData.getCHANNEL().trim();
			} else {
				resp.setRESP_CODE(ResponseCode.ISNULL);
				logger.error("渠道号不能为空！["+reqData.getRESPSEQNO()+"]");
				return resp;
			}
			if (reqData.getBRANCH_ID() != null
					&& !reqData.getBRANCH_ID().trim().equals("")) {
				branch_Id = reqData.getBRANCH_ID().trim();
			} else {
				resp.setRESP_CODE(ResponseCode.ISNULL);
				logger.error("网点号不能为空！["+reqData.getRESPSEQNO()+"]");
				return resp;
			}

			if (reqData.getISSUER_ID() != null
					&& !reqData.getISSUER_ID().trim().equals("")) {
				if (commonUtil.checkIssueId(reqData.getISSUER_ID().trim())) {
					cardholderDTO.setDefaultEntityId(reqData.getISSUER_ID()
							.trim());
					customerDTO
							.setFatherEntityId(reqData.getISSUER_ID().trim());
				} else {
					// 没有找到机构号
			...
			if (reqData.getCARD_NO() == null
					&& reqData.getCARD_NO().trim().equals("")) {
			...
			/* 姓名 */
			if (reqData.getFIRST_NAME() != null
					&& !reqData.getFIRST_NAME().trim().equals("")) {
				if(CharUtil.isChinese(reqData.getFIRST_NAME())){
					...
					logger.error("持卡人姓名包含非法字符！["+reqData.getRESPSEQNO()+"]");
					return resp;
				}
			...
			/* 电话 */
			if (reqData.getCARDHOLDER_MOBILE() != null
					&& !reqData.getCARDHOLDER_MOBILE().trim().equals("")) {
				boolean isTrue = reqData.getCARDHOLDER_MOBILE().trim()
						.matches("([1][3,4,5,8,7][0-9]{9})");
				if (isTrue) {
					customerDTO.setCustomerTelephone(reqData
							.getCARDHOLDER_MOBILE().trim());
					cardholderDTO.setCardholderMobile(reqData
							.getCARDHOLDER_MOBILE().trim());
				} else {
					// 手机号码格式不正确
			...
			/* 证件类型 */
			if (reqData.getID_TYPE() != null
					&& !reqData.getID_TYPE().trim().equals("")) {
				if (reqData.getID_TYPE().trim().equals("1")) {
					...
					logger.error("没有这个证件类型["+reqData.getRESPSEQNO()+"]");
					return resp;
				}
			} else {
			...
			/* 证件号 */
			if (reqData.getID_NO() != null
					&& !reqData.getID_NO().trim().equals("")) {
				if (reqData.getID_TYPE().trim().equals("1")) {
					// 身份证
					String errMsg = IDCardCheck.IDCardValidate(reqData
							.getID_NO().toUpperCase());
					if (errMsg == null || errMsg.equals("")) {
						customerDTO.setCorpCredId(reqData.getID_NO()
								.toUpperCase());
						cardholderDTO.setIdNo(reqData.getID_NO().toUpperCase());
					} else {
						// resp.setRESP_TEXT("证件号有误！");
						resp.setRESP_CODE(ResponseCode.ID_NO_ERROR);
						logger.error("证件号有误["+reqData.getRESPSEQNO()+"]");
						return resp;
					}

				} else if (reqData.getID_TYPE().equals("2")) {
					// 护照
					boolean isTrue = reqData.getID_NO().trim()
							.matches("([0-9a-zA-Z]*)");
					if (isTrue == true) {
			...
			/* 性别 */
			if (reqData.getCARDHOLDER_GENDER() != null
					&& !reqData.getCARDHOLDER_GENDER().trim().equals("")) {
				if (reqData.getCARDHOLDER_GENDER().trim().endsWith("1")
						|| reqData.getCARDHOLDER_GENDER().trim().endsWith("2")) {
					cardholderDTO.setCardholderGender(reqData
							.getCARDHOLDER_GENDER().trim());
				} else {
				...
				if (reqData.getID_TYPE().trim().equals("1")) {
					...
					if (reqData.getID_NO().trim().length() == 18) {
						...
						int num = Integer.parseInt(reqData.getID_NO().trim()
								.substring(16, 17));
						if (num % 2 == 0) {
							// 偶数
			...
			List<EntityStock> stocks = entityStockMapper
					.selectByExample(stockEX);
			if (stocks.size() == 0) {
				// resp.setRESP_TEXT("营销机构库存中没有这张卡！");
				resp.setRESP_CODE(ResponseCode.CARDNO_ISNULL);
				logger.error("没有找到卡["+reqData.getRESPSEQNO()+"]");
				return resp;
			}
			String cardNo = reqData.getCARD_NO();// 报文里的卡号
			
			ProductExample proEX=new ProductExample();
			proEX.createCriteria().andProductIdEqualTo(stocks.get(0).getProductId());
			List<Product> proList=productMapper.selectByExample(proEX);
			//充值金额
			String amt="";
			if (StringUtil.isNumeric(reqData.getTXN_AMT())==false) {
				//整数
				resp.setRESP_CODE(ResponseCode.RECHARGE_AMT_ERROR);
				logger.error("充值金额有误["+reqData.getRESPSEQNO()+"]");
				return resp;
			}else{
				Float txnAmt=Float.parseFloat(reqData.getTXN_AMT());
				if(txnAmt>(Float.parseFloat(proList.get(0).getMaxBalance())/100)){
					resp.setRESP_CODE(ResponseCode.RECHARGE_AMT_ERROR);
					logger.error("充值金额有误,大于限额["+reqData.getRESPSEQNO()+"]");
					return resp;
				}
				BigDecimal bigDecimal=new BigDecimal(reqData.getTXN_AMT());
				BigDecimal bigDecimal1=new BigDecimal("100");
				amt=String.format("%012d", bigDecimal.multiply(bigDecimal1).intValue());
			}
			...
			/* 地址 */
			customerDTO
					.setCustomerAddress(reqData.getDELIVERY_ADDRESS().trim());
			cardholderDTO.setMailingAddress(reqData.getDELIVERY_ADDRESS()
					.trim());
			// /*生日*/
			...
			try {
				if (customerByIdNo != null) {
					...
					customer.setFatherEntityId(customerByIdNo
							.getFatherEntityId());
					...
					CustomerMapper.updateByExampleSelective(customer,
							CustomerEx);
					...
					entityDelivery.setDeliveryAddress(customerDTO
							.getCustomerAddress());
					entityDelivery.setDeliveryName(customerDTO
							.getCustomerName());
					...
					if (example != null && example.size() > 0) {
						...
						deliveryContact.setModifyUser(customerDTO
								.getModifyUser());
					...
					for (ContactDTO contactDTO : ContactDTOs) {
						if (contactDTO.getDataState().trim().equals("1")
								&& contactDTO.getDefaultFlag().trim()
										.equals("1")) {
							contactDTO.setContactName(customer
									.getCustomerName());
							contactDTO.setContactType(customer
									.getCustomerType());
							contactDTO.setContactGender(cardholderDTO
									.getCardholderGender());
							contactDTO.setContactMobilePhone(customer
									.getCustomerTelephone());
							contactDTO.setModifyUser(customerDTO
									.getModifyUser());
							contactDTO.setModifyTime(modifyTime);
							contactDTO.setPapersNo(customerDTO.getCorpCredId());
							contactDTO.setPapersType(customerDTO
									.getCorpCredType());
					...
					if (invoiceAddressDTOs != null) {
						...
						for (InvoiceAddressDTO invoiceAddressDTO : invoiceAddressDTOs) {
							if (invoiceAddressDTO.getDataState().trim()
									.equals("1")
									&& invoiceAddressDTO.getDefaultFlag()
											.trim().equals("1")) {
								InvoiceAddressDTO invoiceAddress = new InvoiceAddressDTO();
								ReflectionUtil.copyProperties(
										invoiceAddressDTO, invoiceAddress);
					...
					if (invoiceCompanys != null) {
						for (InvoiceCompany invoiceCompany : invoiceCompanys) {
							if (invoiceCompany.getDataState().trim()
									.equals("1")
									&& invoiceCompany.getDefaultFlag().trim()
											.equals("1")) {
								invoiceCompany
										.setInvoiceCompanyName(customerDTO
												.getCustomerName());
				...
				commonUtil.updateStockStat(cardNo, defaultEntityId);
			...
			if (null == sellContracts || sellContracts.size() == 0) {
				try {
					contractDTO = sellerContractService
							.insert(sellerContractDTO);
				} catch (Exception e) {
					// TODO: handle exception
					e.printStackTrace();
					commonUtil.updateStockStat(cardNo, defaultEntityId);
					...
					logger.error("[" + reqData.getRESPSEQNO() + "]" + "客户："
							+ customerDTO.getCustomerName() + "添加客户合同失败！");
					throw new Exception(e);
				}
			...
			if (sellProdContractMapper.countByExample(example) == 0) {
				try {
					sellerProductContractService
							.insert(sellerProductContractDTO);
				} catch (Exception e) {
					// TODO: handle exception
					e.printStackTrace();
					commonUtil.updateStockStat(cardNo, defaultEntityId);
					logger.error("[" + reqData.getRESPSEQNO() + "]" + "客户："
							+ customerDTO.getCustomerName() + "添加产品失败！");
					throw new Exception(e);
				}
			}
			...
			sellOrderDTO.setFaceValue(reqData.getTXN_AMT());// 第一次充值金额
			...
			sellOrderDTO.setProcessEntityId(sellerContractDTO
					.getDefaultEntityId());// 营销机构
			...
			sellOrderDTO.setOrderDate(commonUtil.getDateFormat(new Date()));
			sellOrderDTO.setInvoiceDate(commonUtil.getDateFormat(new Date()));
			Float totalPrice = Float.parseFloat(sellOrderDTO.getFaceValue())
					+ Float.parseFloat(packageFee);
			...
			sellOrderInputDTO
					.setLoginUserId(sellerContractDTO.getLoginUserId());
			...
			try {
				...
				// TODO: handle exception
				commonUtil.updateStockStat(cardNo, defaultEntityId);
				orderService.updateOrderStat(sellOrderInputDTO
			...
			orderReadyDTO.setProcessEntityId(sellOrderInputDTO
					.getDefaultEntityId());
			orderReadyDTO.setDefaultEntityId(sellOrderInputDTO
					.getDefaultEntityId());
			...
			try {
				...
				// TODO: handle exception
				e.printStackTrace();
				orderService.delSellOrderCardList(sellOrderInputDTO
						.getOrderId());
				entityStockService.delStockOperaterRecord(sellOrderInputDTO
						.getOrderId());
				commonUtil.updateAccCardInfo(cardNo);// 报文传过来的卡号
				commonUtil.updateStockStat(cardNo, defaultEntityId);
				...
				throw new Exception(e);
				// throw new BizServiceException("售卡订单：" +
			...
			try {
				...
				entityStockService.delStockOperaterRecord(sellOrderInputDTO
						.getOrderId());
				commonUtil.updateStockStat(sellOrderDto.getCardNo(),
			...
			try {
				...
				sellOrderDto.setInvoiceDate(reqData.getBRANCH_ID());//门店号暂存字段
				sellOrderDto.setRefOrder(reqData.getREQSEQNO());//请求流水暂存字段
				sellOrderDto.setBatchNo(amt);//充值金额暂存字段
				submitOrderService.submitOrderAtSendConfirm(sellOrderDto);
				//暂存字段清空
				sellOrderDto.setCvn2(null);// 报文渠道号暂存字段
				sellOrderDto.setInvoiceDate(null);//门店号暂存字段
				sellOrderDto.setRefOrder(null);//请求流水暂存字段
				sellOrderDto.setBatchNo(null);//充值金额暂存字段
			} catch (Exception e) {
				// TODO: handle exception
				e.printStackTrace();
				orderService.delSellOrderCardList(sellOrderInputDTO
			...
			try {
				...
				orderService.updateOrderStat(sellOrderInputDTO.getOrderId());
				logger.error("[" + reqData.getRESPSEQNO() + "]" + "订单："
						+ sellOrderInputDTO.getOrderId() + "激活失败！");
				throw new Exception(e);
			}
			...
			// 发送短信
			SendShortMessageThread msg = new SendShortMessageThread(reqData
					.getCARDHOLDER_MOBILE().trim(), "10043",reqData.getTXN_AMT());
			msg.start();
			// 成功返回
