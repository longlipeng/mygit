...
public class CrmMessageListener implements MessageListener, ApplicationContextAware
{

	private RabbitTemplate crmTemplate;
	private static Logger logger = Logger.getLogger(CrmMessageListener.class);
	...
	public void onMessage(Message message)
	{
		...
		MessageConverter convertor = crmTemplate.getMessageConverter();
		...
		try
		{
			...
			req = (BasicMessage)crmTemplate.getMessageConverter().fromMessage(message);
			...
			if(processor == null)
			{
				...
				crmTemplate.send(reqProps.getReplyTo(), resp);
				return;
			}
			...
			BasicMessage basicMsg=(BasicMessage)crmTemplate.getMessageConverter().fromMessage(message);
...
//				minResp.setRESP_TEXT("系统错误");
	...
	public RabbitTemplate getCrmTemplate() {
		return crmTemplate;
	}



	public void setCrmTemplate(RabbitTemplate crmTemplate) {
		this.crmTemplate = crmTemplate;
	}
