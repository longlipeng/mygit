	...
	public SellerContractDTO insert(SellerContractDTO sellerContractDTO)
		...
		try {
			...
			if (sellerContractDAO.countByExample(example) > 0) {
				throw new BizServiceException("已签过合同，不能重复签合同！");
			}
			...
			//判断是否是批量导入，如果是不需要格式化日期
			if (!"34".equals(sellContract.getContractType()))
			{
			...
			//判断是否是自动化录入，如是自动化录入修改合同类型
			if("34".equals(sellContract.getContractType())){
				sellContract.setContractType(DataBaseConstant.CONTRACT_CUSTOMER);
			}
			sellerContractDAO.insert(sellContract);
			//如果客户状态不是无效，修改客户状态为4
			if (DataBaseConstant.CONTRACT_CUSTOMER.equals(sellerContractDTO
					.getContractType())) {
...
//			}
			...
			if(null==sellerContractDTO.getContractType()||"".equals(sellerContractDTO.getContractType())||!"0".equals(sellerContractDTO.getContractType())){			
				...
				if (!"34".equals(sellerContractDTO.getContractType()))
				{
					 expireDate=DateUtil.getFormatTime(sellerContractDTO.getExpiryDate());
				}else{
					 expireDate = sellerContractDTO.getExpiryDate();
				}
				// 合同状态
