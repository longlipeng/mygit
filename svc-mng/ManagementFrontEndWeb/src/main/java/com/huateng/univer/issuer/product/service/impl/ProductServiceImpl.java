...
public class ProductServiceImpl implements ProductService {
		...
		try {
			...
			for (ServiceDTO acctypeDTO : acctypeDTOs) {
				...
				example.createCriteria()
						.andProductIdEqualTo(productDTO.getProductId())
						.andServiceIdEqualTo(acctypeDTO.getServiceId());
	...
	public void deleteProduct(ProductDTO productDTO) throws BizServiceException {
		try {
			for (ProductDTO productDto : productDTO.getProductDTOs()) {
				...
				productCardBinExample
						.createCriteria()
						.andDataStateEqualTo(DataBaseConstant.DATA_STATE_NORMAL)
						.andProductIdEqualTo(productDto.getProductId())
						.andEffectEqualTo(DataBaseConstant.DEFAULT_FLAG_YES);
				List<ProductCardBin> productCardBinList = productCardBinDAO
						.selectByExample(productCardBinExample);
				...
				if (productCardBinList != null && productCardBinList.size() > 0) {
					...
					if (productCardBin != null) {
						...
						accTypeContractExample.createCriteria()
								.andCardBinEqualTo(productCardBin.getCardBin());
						acctypeContractDAO
								.deleteByExample(accTypeContractExample);
					}
				}
	...
	public PageDataDTO inqueryProduct(ProductQueryDTO productQueryDTO)
		...
		try {
			...
			if (productQueryDTO.getSort() == null
					|| productQueryDTO.getSort().trim().equals("")) {
	...
	public List<ProductDTO> getProductsForUnsignOrder(SellOrderInputDTO sellOrderInputDTO)
			throws BizServiceException {
		try {
			SellOrderDTO orderDto =sellOrderInputDTO.getSellOrderDTO();
			orderDto.setDefaultEntityId(sellOrderInputDTO.getDefaultEntityId());
			List<ProductDTO> productDTOs = baseDAO.queryForList(
					"PRODUCT.selectProductForUnsignOrder", orderDto);
			List<ProductDTO> resultDTOs = new LinkedList<ProductDTO>();
			for (ProductDTO dto : productDTOs) {
				dto = this.viewProduct(dto);
				resultDTOs.add(dto);
	...
	public ProductDTO insertProduct(ProductDTO productDTO)
		...
		try {/*
			...
			logger.info(entityBaseInfoServiceDAO + "@@@@@@@@@@@");
	...
	public ProductDTO viewProductForContract(ProductDTO productDTO)
		...
		try {
			...
			caclDspExample.createCriteria()
					...
					.andDataStatEqualTo(DataBaseConstant.RULE_STATE_ENABLE);
			...
			if (prodFaceValueDTO.getFaceValueType().equals("0")) {
				...
				if (productFaces.size() > 0) {
					throw new BizServiceException(
							(new BigDecimal(prodFaceValueDTO.getFaceValue())
