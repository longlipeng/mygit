...
public class CardBalanceDetailServiceImpl implements CardBalanceDetailService {
	...
	public List<CardBalanceDatailDTO> getAccCardInfo(
		...
		try {
			...
			List<CardBalanceDatailDTO> cardList = baseDAO.queryForList("CARD_BALANCE_DATAIL",
					"GET_CARD_NO_BY_ORDER_TYPE", cardBalanceDatailDTO);
			for (CardBalanceDatailDTO c : hisList) {
				...
				if(c.getTxnType().equals("售卡充值") ){
					for (int i = 0; i < cardList.size(); i++) {
						if(cardList.get(i).getCardNo().equals(c.getCardNo())){
							c.setOrigCardNo(cardList.get(i).getOrigCardNo());
							c.setTxnType("换卡订单");
							c.setDebtOrCredit("");
						}
