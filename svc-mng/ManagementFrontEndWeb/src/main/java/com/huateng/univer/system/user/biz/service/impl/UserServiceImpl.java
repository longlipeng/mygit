...
//			if (!userDTO.getUserId().equals(userDTO.getLoginUserId())) {
//				userDTO.setResetMark("1");//（0，未重置，1重置）
//			}else{
//				userDTO.setResetMark("0");//（0，未重置，1重置）
//			}
			if(null!=userDTO.getUserPassword()&&!userDTO.getUserPassword().equals("")){
				userDTO.setResetMark("1");//（0，未重置，1重置）
			}
			...
			if (user != null) {
				if (!(user.getUserPassword().equals(userDTO
					...
					if (user.getPasswordRecords() != null
						...
						user.setResetMark("0");//密码是否是上一级用户重置0:未重置  1重置
						userServiceDAO.updateByPrimaryKeySelective(user);
					} else {
						user
