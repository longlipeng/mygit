...
public class SellOrderQueryImpl implements SellOrderQuery {
	...
	@Override
	public PageDataDTO queryOther(SellOrderQueryDTO sellOrderQueryDTO)
			throws BizServiceException {
		try {
			return orderBaseQueryBO.queryOtherOrder(sellOrderQueryDTO);
		} catch (Exception e) {
			this.logger.error(e.getMessage());
			throw new BizServiceException("查询订单失败!");
		}
...
//			}
			...
			if (null != sellOrderDTO.getPackageId()
				...
				List<Package> packages = packageDAO
			...
			if (OrderConst.ORDER_TYPE_ORDER_CHANGE_CARD.equals(sellOrderDTO
				...
				sellOrderCompositeDTO.setSellOrderOrigCardList(orderBaseQueryBO
						.queryOrigCardListByOrderbyId(sellOrderCompositeDTO.getSellOrderOrigCardListQueryDTO()));
				/**
				 * 查看明细
				 */
			...
			if(OrderConst.ORDER_TYPE_ORDER_BUY_SELLER_ISSUER_UNSIGN
			    ...
			    //未接收卡明细
			    sellOrderCompositeDTO.setSellOrderNotReceCardList(orderBaseQueryBO
			            .getOrderNotReceiveListPageData(sellOrderCompositeDTO
			                    .getOrderNotReceiveCardListQueryDTO()));
	...
	public List<String> opeCardNoSection(String orderId, String orderType)
		...
		if (orderType
			...
			List<String> oldOrderIdOrderList = baseDAO.queryForList("STOCK_ORDER_CARD_LIST.selectOrigOrderByBuyOrderId", orderId);
            if(oldOrderIdOrderList.size() == 0){
                queryDTO.setOldOrderList(null);
            }
            else{
                queryDTO.setOldOrderList(oldOrderIdOrderList.toArray(new String[oldOrderIdOrderList.size()])); 
            }
			opeList = baseDAO.queryForList(
	 ...
	 */
	@Override
	public SellOrderDownLoadDTO exportExcel(
			SellOrderDownLoadDTO sellOrderDownLoadDTO)
			throws BizServiceException {

		List<SellOrderDownLoadDTO> mailingInfos = null;
		mailingInfos = sellOrderDAO.selectMailingInfos(sellOrderDownLoadDTO);
		if (mailingInfos == null) {
			throw new BizServiceException("无数据可写入");
		}
		List<String> orderIds = new ArrayList<String>();
		List<Map<String, Object>> list = createExcelRecord(mailingInfos, orderIds);
		
		String columNames[] = { "序号", "客户姓名", "电话", "地址", "邮编", "寄达地"};
		String keys[] = { "number", "receiver", "phone", "addr", "postCode", "arriveAddr"};
		...
		fileMap.put(sellOrderDownLoadDTO.getStartDate(), byteList);
		sellOrderDownLoadDTO.setFileData(fileMap);
		//更新订单中下载字段
		if (orderIds.size() > 0){
			sellOrderDownLoadDTO.setOrderIds(orderIds.toArray());
			sellOrderDAO.updateDownloadFalgByOrderId(sellOrderDownLoadDTO);
		}
		return sellOrderDownLoadDTO;
	}

	private List<Map<String, Object>> createExcelRecord(
			List<SellOrderDownLoadDTO> dtos, List<String> orderIds) {
		List<Map<String, Object>> records = new ArrayList<Map<String, Object>>();
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("sheetName", "sheet1");
		records.add(map);
		SellOrderDownLoadDTO dto = null;
		for (int j = 0; j < dtos.size(); j++) {
			dto = dtos.get(j);
			Map<String, Object> record = new HashMap<String, Object>();
			record.put("number", j + 1);
			record.put("receiver", dto.getReceiver());
			record.put("phone", dto.getPhone());
			record.put("addr", dto.getAddr());
			record.put("postCode", dto.getPostcode());
			record.put("arriveAddr", "");
			records.add(record);
