	...
	public void batchCancel(SellOrderDTO sellOrderDTO)
		...
		try {
			...
			for (String orderId : orderIds) {
				...
				orderFlow.setModifyUser(sellOrderDTO.getLoginUserId());
				orderFlows.add(orderFlow);
				/*取消订单要删除临时表的数据 （由于delete 语句如果订单不存在删除是可以的所以在这里不区分是中石油卡还是上汽卡）*/
				UploadCardFileDTO dto = new UploadCardFileDTO();
				dto.setOrderId(orderId);
				cardNoTmp.delete(dto);
			}
			commonsDAO
					.batchUpdate(
							"TB_SELL_ORDER.abatorgenerated_updateByPrimaryKeySelective",
...
//			Integer cardAmt = new Integer(orderList.getCardAmount());
    ...
    /* (non-Javadoc)
     * @see com.huateng.univer.issuer.order.biz.service.StockOrderConfirmService#acceptOrderAll()
     */
    @Override
    public void submitAcceptOrderAll(AcceptOrderDTO acceptOrderDTO) throws Exception {
       
        orderBaseQueryBO.acceptAll(acceptOrderDTO);
    }
