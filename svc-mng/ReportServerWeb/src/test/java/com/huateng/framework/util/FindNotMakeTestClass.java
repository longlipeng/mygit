/**
 * Classname FindClassNotMakeTest.java
 *
 * Version information
 *
...
public class FindNotMakeTestClass extends BaseTest {
	private static Logger logger = Logger.getLogger(FindNotMakeTestClass.class);
	@Test
	public void findNotMakeTest() {
		Set<Class<?>> javaClass = getClasses("com.huateng");
		logger.info("===================================+++++++");
		logger.info("+++++++===================================");
		...
		for (Class<?> clazz : javaClass) {
			...
			if (clazz.getName().indexOf("Test") != -1) {
				MakeTestType mtt = clazz.getAnnotation(MakeTestType.class);
				if (mtt == null) {
			...
			while (dirs.hasMoreElements()) {
				...
				if ("file".equals(protocol)) {
					...
					try {
						...
						logger.error(e.getMessage());
						// log.error("在扫描用户定义视图时从jar包获取文件出错")logger.error(e.getMessage())or(e.getMessage());
					}
				}
			}
		} catch (IOException e){
		...
		for (File file : dirfiles) {
			...
			if (file.isDirectory()) {
				...
				try {
					...
					// log.error("添加用户自定义视图类logger.error(e.getMessage());
					logger.error(e.getMessage());
				}
			}
		}
